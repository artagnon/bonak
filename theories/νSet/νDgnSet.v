...
Definition PaintingPrev {p k} (deps: DepsRestr p.+1 k) : FramePrev deps -> Type :=
  deps.(_paintings).2.
...
(*
Definition mkReflPaintingType {p k} {deps: DepsRestr α p.+1 k}
  `(extraDeps: DepsRestrExtension α p.+1 k deps) :=
  forall q (Hq: q <= k) ε (d: mkFrame α (proj1DepsRestr α deps)),
  deps.(_paintings α).2 (deps.(_restrFrames α).2 q _ ε d) ->
  (mkPaintings α (AddRestrDep α _ deps extraDeps)).2 d.
*)
...
Fixpoint mkReflPaintingTypes {p} :
  forall {k} (deps : DepsReflRestr p k) `(extraDeps: DepsRestrExtension p k deps.(_deps)), Type :=
  match p with
  | 0 => fun _ _ _ => unit
  | S p =>
...
Definition mkReflLayer {p k} (deps: DepsCohs p.+1 k)
  (reflFrames : mkReflFrameTypes deps.(νSet._deps))
  (reflPaintings: mkReflPaintingTypes (toDepsRefl deps reflFrames) deps.(_extraDeps))
  ...
  fun l ε =>
    ...
    reflPaintings.2 (restrFrame 0 leR_O (toDepsRefl deps reflFrames).(_deps) ε d) (l ε).
...
Fixpoint mkCohReflRestrFrameTypesAndReflFrames {p k}:
  ...
  | 0 =>
    ...
    {|
      ...
      ReflFramesDef _ := (tt; fun _ => tt) : mkReflFrameTypes (NextDepsRestr deps)
    |}
  | S p =>
    fun deps reflFrames reflPaintings =>
    let prev := mkCohReflRestrFrameTypesAndReflFrames deps.(1) reflFrames.1 reflPaintings.1 in
    {|
      CohReflRestrFrameTypesDef := mkCohReflRestrFrameTypesStep deps reflFrames prev;
      ReflFramesDef Q :=
...
Definition mkCohReflRestrFrameTypes {p k}
  (deps: DepsCohs p k)
  (reflFrames: mkReflFrameTypes deps.(νSet._deps))
  (reflPaintings: mkReflPaintingTypes (toDepsRefl deps reflFrames) deps.(_extraDeps)) : Type :=
  (mkCohReflRestrFrameTypesAndReflFrames deps reflFrames reflPaintings).(CohReflRestrFrameTypesDef).

(*
Definition restrPainting {p k} (deps : DepsReflCohs p.+1 k) : 
  forall i (Hi: i <= k) (ε : arity) (d : ReflFrameBase (reflOfCohs deps)),
  ReflPaintingBase (reflOfCohs deps) (extraOfCohs deps) d ->
  ReflPaintingPrev (reflOfCohs deps) (restrFrame i Hi (restrOfCohs deps) ε d) :=
  deps.(_depsCohs).(_restrPaintings).2.

Definition reflPainting {p k} (deps : DepsReflCohs p.+1 k) : 
  forall (d : ReflFramePrev (reflOfCohs deps)),
  ReflPaintingPrev (reflOfCohs deps) d ->
  ReflPaintingBase (reflOfCohs deps) (extraOfCohs deps) (reflFrame (reflOfCohs deps) d) :=
...
Definition mkIdReflRestrPaintingType {p k}
  ...
  (reflFrames : mkReflFrameTypes deps.(νSet._deps))
  (reflPaintings : mkReflPaintingTypes (toDepsRefl deps reflFrames) deps.(_extraDeps))
  (idReflRestrFrames : mkIdReflRestrFrameTypes (toDepsRefl deps reflFrames)): Type :=
  forall
    (ε : arity)
    (d : ReflFramePrev (toDepsRefl deps reflFrames))
...
Fixpoint mkIdReflRestrPaintingTypes {p} :
  ...
  | S p =>
    ...
    { _: mkIdReflRestrPaintingTypes deps.(1) reflFrames.1 reflPaintings.1 idReflRestrFrames.1 &T
      mkIdReflRestrPaintingType deps reflFrames reflPaintings idReflRestrFrames }
  end.

Class DepsReflCohs p k := {
  _depsCohs2 : DepsCohs2 p k;
  _reflFrameDeps : mkReflFrameTypes _depsCohs.(νSet._deps);
  _reflPaintingDeps : mkReflPaintingTypes (toDepsRefl _depsCohs _reflFrameDeps) _depsCohs.(_extraDeps);
  _idReflRestrFrames : mkIdReflRestrFrameTypes (toDepsRefl _depsCohs _reflFrameDeps);
...
Definition RestrExtOfReflCohs {p k}
  `(depsCohs: DepsReflCohs p k) : DepsRestrExtension p k (RestrOfReflCohs depsCohs) :=
  depsCohs.(_depsCohs2).(_depsCohs).(_extraDeps).

Definition CohsExtOfReflCohs {p k}
  `(depsCohs: DepsReflCohs p k) : DepsCohsExtension p k (CohsOfReflCohs depsCohs) :=
...
Definition mkReflFrame {p k} (deps : DepsReflCohs p.+1 k):
  FramePrev (NextDepsRestr (CohsOfReflCohs deps)) -> FrameBase (NextDepsRestr (CohsOfReflCohs deps)) :=
  (mkReflFrames deps).2.
...
Definition restrPainting {p k} (deps : DepsReflCohs p.+1 k) :
  ...
  ReflPaintingPrev (ReflOfReflCohs deps) (restrFrame i Hi (RestrOfReflCohs deps) ε d) :=
...
Definition reflPainting {p k} (deps : DepsReflCohs p.+1 k) :
  ...
  ReflPaintingBase (ReflOfReflCohs deps) (RestrExtOfReflCohs deps) (reflFrame (ReflOfReflCohs deps) d) :=
  deps.(_reflPaintingDeps).2.
...
Definition mkIdReflRestrLayer {p k}
  ...
  mkRestrLayer deps.(_depsCohs2).(_depsCohs).(_restrPaintings) (deps.(_depsCohs2).(_depsCohs).(_cohs)) k leR_refl ε _
    (mkReflLayer deps.(_depsCohs2).(_depsCohs) ((ReflOfReflCohs deps).(_reflFrames)) deps.(_reflPaintingDeps) _
...
Proof.
  ...
  rewrite rew_map with
    (P := fun x => deps.(_depsCohs2).(_depsCohs).(νSet._deps).(_paintings).2 x)
    (f := fun x => ((deps.(_depsCohs2).(_depsCohs).(νSet._deps).(_restrFrames).2 k leR_refl ε x))).
  rewrite 2 rew_compose.
  apply rew_swap with (P := fun x => deps.(_depsCohs2).(_depsCohs).(νSet._deps).(_paintings).2 x).
  rewrite rew_app_rl.
  - reflexivity.
  - apply (RestrOfReflCohs deps).(_frames).2.(UIP).
Defined.
...
Fixpoint mkIdReflRestrFrames {p k} (deps : DepsReflCohs p k):
  ...
  - set (h := mkIdReflRestrFrames p k.+1 deps.(1)%depsreflcohs).
    ...
    + intros ε [l c].
      ...
      * exact (h.2 ε l).
      * exact (mkIdReflRestrLayer deps ε l c h).
Defined.
...
Proof.
  ...
  - unshelve econstructor.
    + intros ε.
      exact (rew <- mkIdReflRestrFrame deps ε d in c).
    + exact (L d c).
  - destruct c as [l c'].
    unshelve econstructor.
    + apply mkReflLayer.
      * exact (deps.(_reflPaintingDeps)).
      * exact l.
    + exact (mkReflPainting p.+1 k deps extraDeps (d; l) c').
Defined.
...
Fixpoint mkReflPaintingsPrefix {p k}:
  ...
  mkReflPaintingTypes
    ((NextDepsRefl deps).(1))
    (NextDepsRestr (CohsOfReflCohs deps); mkExtraDeps (CohsExtOfReflCohs deps)) :=
  match p with
  | 0 => fun _ _ => tt
  | S p =>
    fun deps extraDeps =>
...
Definition mkReflPaintings {p k}:
  forall
    (deps : DepsReflCohs p k)
    (extraDeps: DepsReflCohsExtension p k deps),
  mkReflPaintingTypes (NextDepsRefl deps) (mkExtraDeps (CohsExtOfReflCohs deps)) :=
  fun deps extraDeps =>
...
Proof.
  ...
  - destruct deps, _depsCohs3.
    ...
    enough (eq_sym e • e = eq_refl) as ->.
    * reflexivity.
    * apply (mkFrame _).(UIP).
  - unshelve eapply (eq_existT_curried_dep  
      (Q := mkPainting deps .(_depsCohs2) .(_depsCohs) .(_extraDeps))).
    * apply mkIdReflRestrLayer.
    * destruct c as [l c].
      exact (mkIdReflRestrPainting p.+1 k deps extraDeps ε (d; l) c).
Defined.
...
Proof.
  ...
  - unshelve econstructor.
    + exact tt.
    + apply mkIdReflRestrPainting.
  - unshelve econstructor.
    + exact (mkIdReflRestrPaintings p k.+1 deps.(1)%depsreflcohs (deps; extraDeps)%extradepsreflcohs).
    + apply mkIdReflRestrPainting.
Defined.
...
Definition RestrExtOfReflCohs2 {p k}
  `(depsCohs2: DepsReflCohs2 p k) : DepsRestrExtension p k (RestrOfReflCohs2 depsCohs2) :=
  depsCohs2.(_depsReflCohs).(_depsCohs2).(_depsCohs).(_extraDeps).

Definition CohsExtOfReflCohs2 {p k}
  `(depsCohs2: DepsReflCohs2 p k) : DepsCohsExtension p k (CohsOfReflCohs2 depsCohs2) :=
  depsCohs2.(_depsReflCohs).(_depsCohs2).(_extraDepsCohs).

#[local]
Instance proj1DepsReflCohs2 {p k} (depsCohs2: DepsReflCohs2 p.+1 k): DepsReflCohs2 p k.+1 :=
{|
  _depsReflCohs := depsCohs2.(_depsReflCohs).(1);
  _extraDepsCohs2 := (depsCohs2.(_depsReflCohs).(_depsCohs2); depsCohs2.(_extraDepsCohs2));
...
Definition mkCohReflRestrLayer {p k}
  ...
  mkReflLayer
      ...
      deps.(_depsReflCohs).(_reflPaintingDeps) _ deps.(_depsReflCohs).(_cohReflRestrFrames) _
    (mkRestrLayer
  ...
  mkRestrLayer
    ...
    (mkReflLayer
      (mkDepsCohs (Cohs2OfReflCohs2 deps).(1)) (mkReflFrames deps.(_depsReflCohs).(1))
      (mkReflPaintings deps.(_depsReflCohs).(1) (deps.(_depsReflCohs); deps.(_extraDepsRefl))) _ prevCohReflRestrFrames d l).
Proof.
  apply functional_extensionality_dep.
  intros θ.
  unfold mkRestrLayer, mkReflLayer.
  ...
  rewrite rew_map with
    ...
    (f := fun x => (NextDepsRestr (CohsOfReflCohs2 deps).(1)).(_restrFrames).2 0 leR_O θ x).
  rewrite rew_map with
    (P := fun rf => ReflPaintingBase _ _ rf)
    (f := fun d0 => reflFrame (toDepsRefl _ _) d0).
  rewrite rew_map with
    (P := fun b => (mkDepsCohs (Cohs2OfReflCohs2 deps)).(1).(νSet._deps).(_paintings).2 b)
    (f := fun d0 => (mkDepsCohs (Cohs2OfReflCohs2 deps)).(1).(νSet._deps).(_restrFrames).2 i (↑ Hi) ε d0).
  rewrite 4 rew_compose.
  apply rew_swap with
    (P := fun b => (mkDepsCohs (Cohs2OfReflCohs2 deps)).(1).(νSet._deps).(_paintings).2 b).
  rewrite rew_app_rl.
  - reflexivity.
  - apply (mkFrame (RestrOfReflCohs2 deps).(1)).(UIP).
Defined.
...
Proof.
  ...
  - set (h := mkCohReflRestrFrames p k.+1 deps.(1)%depsreflcohs2).
    ...
    + intros i Hi ε [l c].
      unshelve eapply eq_existT_curried.
      * exact (h.2 i.+1 (⇑ Hi) ε l).
      * exact (mkCohReflRestrLayer deps ε i Hi l c h).
Defined.
...
Proof.
  unshelve econstructor.
  - unshelve econstructor.
    + exact (mkDepsCohs (Cohs2OfReflCohs2 deps)).
    + exact (mkExtraCohs deps.(_extraDepsCohs2)).
    + apply mkCohPaintings.
  - apply mkReflFrames.
  - exact (mkReflPaintings _ (deps.(_extraDepsRefl))).
  - apply mkIdReflRestrFrames.
  - apply mkIdReflRestrPaintings.
  - apply mkCohReflRestrFrames.
Defined.
...
Proof.
  ...
  - apply (AddReflCohDep (mkDepsReflCohs deps)).
    exact (mkExtraReflCohs p.+1 k deps extraDeps).
Defined.

Fixpoint mkCohReflRestrPainting {p k}
  (deps : DepsReflCohs2 p k)
  (extraDeps : DepsReflCohs2Extension p k deps):
  mkCohReflRestrPaintingType (mkDepsReflCohs deps) (mkExtraReflCohs deps extraDeps).
Proof.
  intros ε i Hi d [l c].
  destruct i.
  ...
  - unfold ReflPaintingBase, PaintingBase, toDepsRefl; simpl.
    ...
    + unshelve eapply (eq_existT_curried_dep
      ...
      * apply (mkCohReflRestrLayer deps ε i (⇓ Hi) d l).
      * exact (mkCohReflRestrPainting p.+1 k deps extraDeps ε i (⇓ Hi) (d; l) c).
Defined.

Fixpoint mkCohReflRestrPaintings {p k}
  (deps : DepsReflCohs2 p k)
  (extraDeps : DepsReflCohs2Extension p k deps):
  mkCohReflRestrPaintingTypes (mkDepsReflCohs deps) (mkExtraReflCohs deps extraDeps).
Proof.
  destruct p.
  - unshelve econstructor.
    + exact tt.
    + apply mkCohReflRestrPainting.
  - unshelve econstructor.
    + exact (mkCohReflRestrPaintings p k.+1 deps.(1)%depsreflcohs2 (deps; extraDeps)%extradepsreflcohs2).
    + apply mkCohReflRestrPainting.
Defined.
...
Definition dgnDepsReflCohs2FromData {p}
  ...
  {|
    ...
    _extraDepsRefl := TopReflCohDep (deps := dgnDepsReflCohsFromData C E E' D) L;
...
Definition mkνSetData0 : νSetData 0 :=
  {|
    ...
    restrFrames := (tt : mkRestrFrameTypes (tt : mkPaintingTypes 0 0 (tt : mkFrameTypes 0 0)));

...
\usepackage{amsmath, amssymb, mathrsfs, url, bookmark, tikz, tikz-cd, mathpazo, enumitem, xargs, multirow, float, tabularx, environ, booktabs, setspace, makecell, colortbl, pdflscape, flushend, minted, art.cls/colorpal, art.cls/ct, art.cls/sset, art.cls/lim, art.cls/joinargs}
...
% - semisimplicial/semicubical sets in indexed form
%   - clarify indexed/fibered, justify advantages of the respective choice
%     - the presentation matters when expressed in indexed way!
%   - compare to indexed VV, previous indexed HH, indexed Pang-Luo
% - iterated unary/binary parametricity has a semi-simplicial/semi-cubical structure
% - nu-types as a uniform definition of semi-simplicial/semi-cubical
%   iterated parametricity gives an alternative presentation of indexed semi-simplicial/semi-cubical
%   this is what we described in the paper
% - perspective: models of cubical type theory using indexed cubical sets, hence preserving more definitial rules
% - mention also the semi-simplicial types problem

\subsection*{Fibered vs indexed presentation of semi-simplicial and semi-cubical sets}
A family of sets can commonly be represented in two ways: as a family properly speaking, indexed over a set $S$, or as a set $T$ together with a map specifying for each element of the set $T$ on which index it depends in $S$. In the first case, we call it an \emph{indexed} presentation. In the latter case, the set associated to a given index is the fiber, i.e. inverse image, of the index under the map, so we call it a \emph{fibered} presentation. The two presentations are equivalent and the equivalence can be phrased concisely in the language of homotopy type theory~\cite{hottbook} as a fibered/indexed correspondence\footnote{At a discussion on the Zulip channel for the HoTT community, alternative names fibration/family correspondence and unbundled/bundled correspondence were proposed, among others. In our experience, the opposition fibered/indexed is spontaneously understood by many listeners. In particular, it echoes to the Grothendieck construction of fibered categories from indexed categories. The most elementary instance of the correspondence, with $\Type$ instead of $\U$, that is $(\Sigma S: \Type. (S \rightarrow T)) \simeq (T \rightarrow \Type)$, is sometimes called ``Grothendieck construction for dummies'' (its proof requires univalence, see~\cite{hottbook}).}
\begin{equation*}
  \mbox{(fibered)}\qquad(\Sigma S: \U. (S \rightarrow T)) ~\simeq~ (T \rightarrow \U)\qquad \mbox{(indexed)}
\end{equation*}
where $\U$ represents sets in homotopy type theory as the subset of types with a unique proof of equality (within some universe).

A \emph{presheaf} on an category is a family of sets connected with maps according to the morphisms of the category. As such, it lives on the indexed side of the correspondence, opposing on the fibered side to what is called \emph{discrete Grothendieck fibrations}. However, there are situations where a presheaf $F$ can also be seen as living on the fibered side of the correspondence. Indeed, when the indexing category is direct, i.e. with a downwards-well-founded collection of non-identity morphisms, as is the case of semi-simplicial and semi-cubical sets, each $F(X)$ serving as source of a map $F(X) \stackrel{F(f)}{\rightarrow} F(Y)$ can be turned into a family $F(X)$ indexed on $F(Y)$, and, since the maps are well-founded downward, this can be iterated from below, eventually producing a collection of families which we call an \emph{indexed presentation} of the presheaf over the direct category.

For instance, in the case of a semi-cubical set~\citep{grandis03,buchholtz17}, presented with $2n$ face maps from the set of $n$-cubes to the set of $(n-1)$-cubes, the standard presheaf approach prescribes a family of sets fibered the ones over the others (up to cubical faces identities). Formulated in type theory, it corresponds to:
...
On the other side, the indexed presentation of the same presheaf is a stratified sequence of families indexed on elements of families of lower rank, taking into account the coherence conditions to prevent duplications. Formulated in type theory, it takes the form:
...
The idea for such indexed presentation of presheaves over a direct category was mentioned at the Univalent Foundations year in the context of defining semi-simplicial types~\footnote{See e.g. \url{https://ncatlab.org/nlab/show/semi-simplicial+types+in+homotopy+type+theory}.}. A couple of constructions were proposed since then. One, by \cite{voevodsky12}, relies on the presentation of semi-simplicial sets as a presheaf over increasing injective maps between finite ordinals. Another, by \cite{herbelin15}\footnote{With hindsight, the title of the paper ``A dependently-typed construction of semi-simplicial types'' is somehow confusing: it was part of a project at defining semi-simplicial types, but, in practice, the definition is done in a type theory with Unicity of Identity Proofs (UIP), meaning in a type theory where types (intended to represent arbitrary $\infty$-groupoids) are actually sets ($0$-truncated $\infty$-groupoids, that is, morally, $0$-groupoids). The confusion was however common at that time.}, and fully formalized in Coq, relies on the presentation of semi-simplicial sets as a presheaf over face maps. Another by~\cite{part15}, formalized in (an emulation of logic-enriched homotopy type theory in) the Plastic proof assistant, and yet another by \cite{annenkovCK17}, formalized in (an emulation of a two-level type theory in) the Agda proof assistant~\footnote{\url{https://github.com/nicolaikraus/HoTT-Agda/blob/master/nicolai/SemiSimp/SStypes.agda}}, also rely on the presentation of the semi-simplicial category from increasing injective functions over finite ordinals.

The indexed definition of a presheaf over a direct category is technically more involved than the presheaf definition as it requires hard-wiring in the structure the dependencies between elements of the sets of the presheaf, including the coherence conditions between these dependencies, such as taking the $i$-th face of the $j$-th face of a $n$-simplex is the same as taking the $(j-1)$-th face of the $i$-th face (when $j>i$). On the other side, defining a concrete presheaf in indexed form only requires to provide the family since the responsibility of defining maps and showing the coherence conditions is already accounted for in the definition, what is not the case by default in the (ordinary) presheaf presentation.

In category-theoretic terms, the indexi set of the family $X_n$ in the indexed presentation is called a \emph{matching object}. The indexed presentation of presheaves over a direct category can then be identified with a subclass of ordinary presheaves known as Reedy fibrant where Reedy fibrancy precisely expresses that the set $X_n$ in the fibered presentation is a $\Sigma$-type over an instance up to isomorphism of the matching object at level $n$~\citep{shulman14,kraus17}.
...
In this section, we generalize semi-simplicial and semi-cubical sets to, what we call, $\nu$-sets, subsuming the earlier definitions. However, we also explicitly work out the cases of semi-simplicial and semi-cubical sets, in a way that is similar to a previous art~(\citeyear{aitchison86}).
...
%% Notations
% simplicial, augmented, where
% Γ           Γ+         Augmented Simplicial Combinatorics through Category Theory
%                        José Manuel García-Calcines, María Teresa Rivas-Rodríguez, Luis Javier Hernández-Paricio, 2022

% semi-augmented, where
% ∆_s        HOMOLOGY IN RELATIVE SEMI-ABELIAN CATEGORIES, JULIA GOEDECKE, 2011

% simplicial, semi-simplicial, where
% Δ           Δ_inj            SEMI-SIMPLICIAL SPACES, JOHANNES EBERT AND OSCAR RANDAL-WILLIAMS, 2018
% Δ           Δ_inj            PhD, Josep Esquirol Esteve, 2019
% Δ           Δ'               Strict n-categories and augmented directed complexes model homotopy types, Andrea Gagna, 2018
% Δ           Δ_inj, Δ_{inj,aug} Ms Th. Jonas Dallend ̈orfer, 2019
...
\begin{definition}[$\nu$-semi-shape categroy]
  ...
  \begin{figure}[H]
    ...
    \begin{align*}
      \Obj       & := \Nat                                                                            \\
      \Hom(p, n) & := \{l \in (\nu \cup \kstar) \mid \text{number of $\kstar$ in $l = p$}\}           \\
      g \circ f  & :=
      \begin{cases}
        f                  & \text{if $g = []$}                                                         \\
        a :: (g' \circ f)  & \text{if $g = (a :: g')$}, \text{where $R \in \nu$}                        \\
...
%     - standard augmented triangle:
%       Inductive points := A | B | C.
%       Inductive lines := AB | BC | AC.
%       Inductive surface := ABC.

%       {X n := match n with 0 => unit | 1 => points | 2 => lines | 4 => surface | n => empty_set;
%        d {n} i (Hi:i<n) (x:X n) := match n, i, x with
%        | 0, _, _ => assert false
%        | 1, _, _ => tt
%        | 2, 0, AB => B
%        | 2, 1, AB => A
...
%        | 3, 1, ABC => AC
%        | 3, 2, ABC => AB
%        | n.+4, i, _ => assert false
%        end;
%        alpha n i j (Hi:i<n) (Hj:j<i) (x : X n) : d i Hi (d j _ x) = d (j-1) _ (d i _ x)
%        := match n, i, j, x with
%        | 0, _, _, _ => assert false
%        | 1, _, _, _ => assert false
%        | 2, _, _, _ => proof_irrelevance_of_unit : tt = tt
%        | 3, 1, 0, ABC => proof_irrelevance_in_points : C = C
%        | 3, 2, 0, ABC => proof_irrelevance_in_points : B = B
%        | 3, 2, 1, ABC => proof_irrelevance_in_points : A = A
%        | n.+4, i, _ => assert false
%        end
%        }
%        {hd tt := unit;
%         tl := {hd _ (* (tt;tt) *) := points; (* 3 points *)
%                tl := {hd _ (* ((tt;(tt;a));b) *) := unit; (* unique line over any a and any b *)
%                       tl := {hd _ (* ((tt;(tt;(a;(b;tt))));((c;tt);tt)) *) := unit; (* unique triangle over ab, bc, and ac *)
%                              tl := cofix s := {hd _ := empty_set; tl := s}}}}}
%        {hd _ := unit; tl :=
%        {hd _ := points; tl :=
%        {hd _ := unit; tl :=
%        {hd _ := unit; tl := cofix s := {hd _ := empty_set; tl := s}}}}} : AugmentedSemiSimplicial

%        standard n : AugmentedSemiSimplicial
%        := {hd _ := unit; tl :=
%           {hd _ := [0;n]; tl :=
%            unit unit unit ... n times ... unit; empty_set; ... }}
...
In Section~\ref{sec:ett} and Tables~\ref{tab:coind}, \ref{tab:core}, \ref{tab:frames}, \ref{tab:faces}, \ref{tab:coh}, we give an informal, mathematical definition in Extensional Type Theory, which roughly corresponds to a level of reasoning ``à la set theory'' where equalities are thought as ``holding on the nose'' without having to mention them explicitly once they are justified. In Section~\ref{sec:itt}, we give the same mathematical definition in Intensional Type Theory, where transporting along coherence conditions is now made explicit. Adding this level of details helps to understand the specificities of coherence conditions.
...
\subsection*{Formal construction in ETT\label{sec:ett}}
...
Note that for a fixed constant $n$, the coherence conditions evaluate to a reflexivity proof, so that the construction evaluates to an effective sequence of types of iterated relations not mentioning them anymore. For instance, it produces Fig.~\ref{fig:intuition} for $n=2$ and $\nu=2$.

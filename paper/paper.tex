...
\usepackage{multirow, minted, tabularx, environ, booktabs, setspace, makecell, caption, art.cls/joinargs, colortbl, pdflscape}
...
\begin{equation*}
  ...
  \framebox{$\begin{array}{lll}
        ...
        \framebox{$
            \begin{array}{ll}
               & \framebox{$a_{1\unitpoint}$}         \\
              \framebox{$a_{10}$}~\framebox{$a_{11}$} \\
            \end{array}
          $}~
        \raisebox{4.8mm}{$\framebox{$
                \begin{array}{ll}
                   & \framebox{$a_{\unitpoint\unitpoint}$}                    \\
                  \framebox{$a_{\unitpoint 0}$}~\framebox{$a_{\unitpoint 1}$} \\
                \end{array}
              $}$}
      \end{array}$}
\end{equation*}

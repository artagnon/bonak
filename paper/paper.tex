...
If we think of $X_1$ fibered over two copies of $X_0$ in an indexed way, we get $X_1$ of type $X_0 \times X_0 \rightarrow \U$. Continuing the process, we can think of $X_2$ fibered over four copies of $X_1$ themselves fibered over two copies of $X_0$ as a dependent type. We have a priori a dependency over eight copies of $X_0$ but these are actually four due to the coherence conditions. Based on these coherence conditions, the type of $X_2$ happens to have the form $\Pi a b c d, X_1 (a, b) \times X_1 (a, c) \times X_1 (b, d) \times X_1 (c, d) \rightarrow \U$, recovering the square structure of $X_2$. In summary,

\begin{align*}
  X_0 & :                  & \U                                                       \\
  X_1 & :                  & X_0 \times X_0 \rightarrow \U                            \\
  X_2 & : \Pi a b c d, & X_1 (a, b) \times X_1 (a, c)  \times X_1 (b, d) \times X_1 (c, d) \\
 & &  \rightarrow \U                    \\
  \ldots
\end{align*}

Now, let's observe that the process of construction of the type of
$X_1$ from that of $X_0$ and from that of $X_2$ to that of $X_1$ is
similar to applying a binary parametricity translation on contexts that
is: to triplicate each declaration $x_L:X_0$ and $x_R:X_0$ into two
copies of $x$ and a line connecting these copies, that is into two
triples of declarations $x_{LL}:X_0, x_{LR}:X_0,
x_{L\kstar}:X_1(x_{LL},x_{LR})$ and $x_{RL}:X_0, x_{RL}:X_0,
x_{R\kstar}:X_1(x_{RL},x_{RR})$, and to expand $\U$ into the type of a
relation over these two copies, that is, into $x_{\kstar
  L}:X_1(x_{LL},x_{RL}) \times x_{\kstar R}:X_1(x_{LR},x_{RR})
\rightarrow \U$.
...
In the cubical case, we expect the construction to eventually provide
a model of (some version of) parametric type theory~\cite{} by adding
degeneracies, a hierarchy of universes (as sketched e.g. in a talk by
Herbelin at the HoTT-UF workshop for the bridge
case~\cite{herbelin-hott-uf}) as well as reasoning modulo
permutations~\cite{grandis03}.

By equipping the universe construction with a structure of
equivalences (as suggested along the lines of Altenkirch and
Kaposi~\cite{}), as well as connections~\cite{}, we expect the
parametric model to extend into a model of cubical type
theory~\cite{}. A key point in this approach, is that, thanks to the
indexed construction, we expect univalence to hold by definition in
the model, without requiring any kind of Reedy fibrancy as it would a
priori be needed in the fibered case. If these intuitions are correct,
this would definitively root cubical type theory on top of iterated

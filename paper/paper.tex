...
\maketitle
\begin{abstract}
  Constructions such as semi-simplicial and semi-cubical sets can be defined in the ``usual way'' as presheaves over respectively the semi-simplex or semi-cube category, which we shall call \emph{fibered} definitions, but also, as done e.g. in Voevodsky~\cite{voevodsky12} and in previous work~\cite{herbelin15} as a dependently-typed, or, say, \emph{indexed} construction.
...
We can iteratively apply the above correspondence to obtain an indexed representation of semi-cubical sets.

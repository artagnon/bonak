...
and $\restrp[n-1][p]$ as references and renaming them respectively as
$\framep[n][p]$, $\painting[n][p]$, $\restrf[n][p]$ and
$\restrp[n][p]$, replace the equational properties of $\framep[n][p]$,
$\framep[n-1][p]$, $\restrf[n][p]$, $\painting[n][p]$,
$\painting[n-1][p]$ and $\restrp[n][p]$ as definitions of objects
respectively renamed to $\framep[n+2][p]$, $\framep[n+1][p]$,
$\restrf[n+1][p]$, $\painting[n+2][p]$, $\painting[n+1][p]$ and
$\restrp[n+1][p]$, building instead by induction a block of the form:
...
\begin{align*}
  ...
  \begin{array}{llcl}
    ...
    \left(\begin{array}{l}
              ...
              \restrp[n+1][p](\depscoht.\fulldepscohs[n][0,...,n])              \\
...
\begin{itemize}
  ...
  \item $\restrp[n+1][0,...,n]$ assuming $E':\framep[n+2][n+1](\overline{\deps}(\depscohs[n][0,...,n-1])) \rightarrow \U$ comes from $\fulldepscohs[n][0,...,n-1]$ which holds by assumptions (additionally using $E'$)

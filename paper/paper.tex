...
% Adjust font size
\newcommand{\smfontsize}{\fontsize{8}{10}\selectfont}
...
An informal description of what should semi-simplicial sets in indexed form be was given in~\cite{IAS}. Several implementations in type theory were later proposed~\cite{...}. In \cite{hr25}, a construction of both augmented semi-simplicial and semi-cubical sets in \emph{indexed form} was given, as a special case of iterated parametricity in indexed form. Instead of the following illustration of semi-cubical sets in fibered form:
...
In particular, the dependency of $\framep[n+1][\lbrack 0,p-1 \rbrack]$ within the type of $\restrf[n][\lbrack 0,p-1 \rbrack]$ combined with the dependency of $\restrf[n][\lbrack 0,p-1 \rbrack]$ within the definition of $\framep[n+1][\lbrack 0,p \rbrack]$ requires to mutually define the type of $\restrf[n][\lbrack 0,p-1 \rbrack]$ together with $\framep[n+1][p]$ (or $\framep[n+1][\lbrack 0,p \rbrack]$) as a \emph{function} from $\restrf[n][\lbrack 0,p-1 \rbrack]$.
...
\begin{itemize}
  ...
  \item In section \ref{sec:detailed-construction}, we left implicit the management of inequality proofs occurring in restrictions and coherence conditions. In practice, we stated inequalities as strict propositions~\cite{}, that isas propositions whose proofs are all definitionally equal. This simplified a bit the formalisation.
...
\label{sec:previous-formalisation}

...
We previously exploited the observation that semi-simplicial and semi-cubical sets are naturally unary and binary cases of iterated parametricity~\cite{hr25}. In adding a new dimension to our construction, we observed the usual cubical degeneracies in the cubical case and \emph{connections}~\cite{bernardy12,bernardy15} in the simplicial case. The coherence conditions we used to complete our construction made explicit the ones outlined in Grandis-Mauri~\cite{grandis03}. Our work includes a mechanically-checked formalization in Rocq~\href{https://github.com/artagnon/bonak}{\linkicon}.
...
In particular $1$-semi-shape category presents the semi-simplicial category while the $2$-semi-shape category presents the semi-cubical category (note that this also relates to unary and binary parametricity~\cite{reynolds83,bernardy15}).

Degeneracies can be added to the $\nu$-semi-shape category in different ways. We here focus on cubical-style degeneracies, as e.g. in Grandis~\cite{grandis03} and their adaptation to the unary case as done in \cite{bernardy15}. A morphism from $p$ to $n$ in the $\nu$-shape category is the composition of a $q$-long degeneracy from $p$ to $p-q$ followed by a face map from $p-q$ to $n$.

\begin{definition}[$\hexagon_d$]
  The definition of $\nu$-shape category extends the $\nu$-shape category with degeneracies:
  \begin{align*}
    \Obj_{\hexagon_d}       & := \Nat                                                                                       \\
    \Hom_{\hexagon_d}(p, n) & := \{(q,l) \mid l \in (\nu \sqcup \{\kstar\})^n \mid \text{number of $\kstar$ in $l = p-q$}\} \\
    (0,g) \circ (q,f)       & := \left(q,
    \begin{cases}
      f                & \text{if $g = \epsilon$}                                                   \\
      a\,(g' \circ f)  & \text{if $g = a\,g'$}, \text{where $a \in \nu$}                            \\

...
Parametricity, like realisability, reducibility, and logical relations, was most commonly handled in an \emph{indexed} form, that is, using a type-theoretic language, considering predicates of type $A \rightarrow \Prop$, or families of type $A \rightarrow \MySet$, or relations of type $A \times A \rightarrow \Prop$, or relevant relations -- sometimes called \emph{correspondences} -- of type $A \times A \rightarrow \MySet$, where $\Prop$ and $\MySet$ are types representing, that is classifying -- in category theory terminology --, propositions or sets, respectively. Along the indexed-fibered correspondence, that is along the equivalence between the types $A \rightarrow \Prop$ and $\Sigma B:\MySet. (B \stackrel{inj}{\rightarrow} A)$ (proof-irrelevant case) and $A \rightarrow \MySet$ and $\Sigma B:\MySet. (B \rightarrow A)$ (proof-relevant case), for $A:\MySet$, everything told in the indexed form can morally also be told in \emph{fibered} form. Such fibered approach to parametricity was for instance considered in Ghani \cite{Ghani...}, who also suggested to iterate parametricity.

\paragraph*{Iterated parametricity.}

The internalisation of parametricity in unary indexed form was studied by \cite{Moulin,Bernardy} directly leading to a type theory of iterated parametricity. Such type theory could be interpreted in a ``unary'' form of cubical set~\cite{BernardyCoquandMoulin}, thus implicitly in fibered form since, in a binary cubical set (resp. in a unary cubical set and simplicial set as well), a set of segments over two points, that is a relevant relation, is given by first and second projection rather than as a function to $\MySet$ (resp. a family is given as a \emph{total space} projected over a set of points $A:\MySet$ rather than a family of type $A \rightarrow \MySet$. This connection between iterated binary parametricity in fibered form and cubical sets includes an identification of degeneracies and reflexivity proofs, which can also be found in Johann and Sojakova~\cite{JohannSojakova}. In particular, it became clear at this point that iterated binary parametricity (without reflexivity) produces semi-cubical sets while it was also observed\footnote{The authors learned it from Hugo Moeneclaye and Thorsten Altenkirch.} that iterated unary parametricity produces augmented semi-simplicial sets.

$$
\begin{tabular}{lllllll}
                           &                            &                                 &                   & structure & kind of semantics \\
 \hline
 \multirow{6}{1cm}{unary}  & \multirow{3}{1.5cm}{indexed} & \multirow{2}{2cm}{non-iterated} & proof-irrelevant  & predicate &        reducibility \\
                           &                            &                                 & relevant          & family & \\
                           &                            & \multicolumn{2}{l}{iterated}                        & \emph{this work}       & \\
                           & \multirow{3}{1.5cm}{fibered} & \multirow{2}{2cm}{non-iterated} & proof-irrelevant  & subobject & \\
                           &                            &                                 & relevant          & fibration & \\
                           &                            & \multicolumn{2}{l}{iterated}                        & augmented semi-simplicial set & \\
 \hline
 \multirow{6}{1cm}{binary} & \multirow{3}{1.5cm}{indexed} & \multirow{2}{2cm}{non-iterated} & proof-irrelevant  & relation & logical relation \\
                           &                            &                                 & relevant          & correspondence (relevant relation) & \\
                           &                            & \multicolumn{2}{l}{iterated}                        & \emph{this work}       & \\
                           & \multirow{3}{1.5cm}{fibered} & \multirow{2}{2cm}{non-iterated} & proof-irrelevant  & fibered relation ? & \\
                           &                            &                                 & relevant          & span & & \\
                           &                            & \multicolumn{2}{l}{iterated}                        & semi-cubical set & \\
 \hline
\end{tabular}
$$

In a previous paper~\citep{hr25}, we exploited this correspondence to construct at the same time augmented semi-simplicial and semi-cubical sets in \emph{indexed form} as a special case of iterated parametricity. That is, instead of the following illustration of the semi-cubical case (upto faces identities),
...
\begin{equation*}
  ...
  \end{tikzcd}
\end{equation*}
we consider a family of relevant relations as shown below:
\begin{equation*}
  \begin{array}{lll}
    E_0 & :               \U                                                                          \\

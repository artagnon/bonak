...
\begin{example}[$\DeltaPlus^1$]
  In dimension $1$, the standard augmented semi-simplex can be pictured as a point, where the unique color is kept implicit; the unique morphism in $\Hom(0, 1)$ is not pictured. This point is the identity in $\Hom(1, 1)$; it is hence shown as a singleton $\kstar$:
...
\begin{example}[$\DeltaPlus^2$]
  In dimension $2$, the standard augmented semi-simplex is drawn as two points, given by $\Hom(1, 2)$, along with a line connecting them, given by $\Hom(2, 2)$. Again, the unique morphim in $\Hom(0, 2)$ is not pictured.
...
Homotopy type theory (HoTT) is a logical foundation based on Martin-Löf type theory and the univalence axiom~\cite{hottbook}. HoTT is based on the notion of a basic \textsf{Type}, and the notion of sets can be recovered as \textsf{HSet}, where \textsf{HSet} is a straightforward packaging of two fields:
...
In the \textsf{HSet} universe, three properties hold:
...
\textsf{HSet} provides us a form of unicity of identity of proofs (UIP), which is a form of proof irrelevance:
...
The latter correspondence can be expressed in HoTT, for $T: \textsf{Type}$, as an equivalence between $\Sigma S: \textsf{Type}\;.\;S \rightarrow T$ and $T \rightarrow \textsf{Type}$, which can be proved using univalence. This correspondence also holds on \textsf{HSet}, for $T: \textsf{HSet}$:

\begin{equation*}
  \Sigma S: \textsf{HSet}\;.\;S\rightarrow T \simeq T \rightarrow \textsf{HSet}
\end{equation*}

...
Parametricity, like realisability, reducibility, and logical relations, was most commonly handled in an \emph{indexed} form, that is, using a type-theoretic language, considering predicates of type $A \rightarrow \Prop$, or families of type $A \rightarrow \U$, or relations of type $A \times A \rightarrow \Prop$, or relevant relations -- sometimes called \emph{correspondences} -- of type $A \times A \rightarrow \U$, where $\Prop$ and $\U$ are types representing, that is classifying -- in category theory terminology --, propositions or sets, respectively. Along the indexed-fibered correspondence, that is along the equivalence between the types $A \rightarrow \Prop$ and $\Sigma B:\U. (B \stackrel{inj}{\rightarrow} A)$ (proof-irrelevant case) and $A \rightarrow \U$ and $\Sigma B:\U. (B \rightarrow A)$ (proof-relevant case), for $A:\U$, everything told in the indexed form can morally also be told in \emph{fibered} form. Such fibered approach to parametricity was for instance considered in Ghani \cite{Ghani...}, who also suggested to iterate parametricity.

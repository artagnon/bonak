...
Equations specifying $\nu$-sets in indexed form were given in the work with an accompanying formalisation in Rocq. The well-foundedness of the construction was by induction on large structure involving equational reasoning. The current work presents a refined construction which we consider expresses the computational recursive essence of the construction.
...
Using this recursive definition of $\framep[n+1][\lbrack 0,p \rbrack]$ and $\restrf[n][\lbrack 0,p-1 \rbrack]$, we are thus able to extend the specification of the structure with its third component, for $p \leq n$.
...
The specification of the six fields being complete, the induction step can be built. All of $\framep[n+1][\lbrack 0,p \rbrack]$, $\painting[n+1][\lbrack 0,p \rbrack]$, $\restrf[n+1][\lbrack 0,p \rbrack]$ and $\restrp[n+1][\lbrack 0,p \rbrack]$ have already been built, so only the construction of $\cohf[n+1][\lbrack 0,p \rbrack]$ and $\cohp[n+1][\lbrack 0,p \rbrack]$ remains to be done. Details follow.
...
\begin{equation*}
  ...
  \end{array}
\end{equation*}
where a transport of $\coht$ on the left, functoriality of transport, transitivity of equality and functional extensionality relative to $\theta$ were used to go from the following type\footnote{omitting the $\framep$ argument} of $(\ap \overrightarrow{\cohf[n][p][r][0][\omega][\theta]\ldots}(\ap \overrightarrow{\ap\restrf[n][p][r][\omega](\cohf[n+1][p][q+1][0][\epsilon][\theta]\ldots)}(\cohp[n][p][q][r][\epsilon][\omega](l_\theta))))$
\begin{equation*}
  \smfontsize
  \begin{array}{lcl}
    \begin{array}{l}
  ...
  \end{array}
\end{equation*}
to this unfolding of the type of $\cohl[n][p][q][r][\epsilon][\omega]$
\begin{equation*}
  \smfontsize
  \begin{array}{lcl}

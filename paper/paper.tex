...
Homotopy type theory (HoTT) is a logical foundation based on Martin-Löf type theory and the univalence axiom~\cite{hottbook}. HoTT is based on the notion of a basic \textsf{Type}, and the notion of sets can be recovered as \textsf{HSet}, where \texttt{HSet} is a straightforward packaging of two fields:
...
In HoTT, the latter correspondence can be expressed, for $T: \textsf{Type}$, as an equivalence between $\Sigma S: \textsf{Type}\;.\;S \rightarrow T$ and $T \rightarrow \textsf{Type}$, which can be proved using univalence. This correspondence also holds on \textsf{HSet}, for $T: \textsf{HSet}$:

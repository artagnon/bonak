...
\begin{align*}
  ...
  \restrf[n][p] & : \Pi q \leq n+p.\, \framep[n+1][p] \rightarrow \framep[n][p] & \mbox{for all $p\leq n$}                         \\
  \restrp[n][p] & : \Pi q \leq n+p.\, \Pi d:\framep[n+1][p]\, \painting[n+1][p](d) \rightarrow \painting[n][p](\restrf[n][p](d)) & \mbox{for all $p\leq n$} \\
  ...
  \cohp[n][p]     & \Pi qr\epsilon\omega.\,\Pi d:\framep[n+2][p].\, \restrp[n][p][q][\epsilon](\restrf[n+1][p][r][\omega](d)) \circ \restrp[n+1][p][r][\omega](d) = \\
                  & \;\;\;\;\restrp[n][p][r][\omega](\restrf[n+1][p][q+1][\epsilon](d)) \circ \restrp[n+1][p][q+1][\epsilon](d) & \mbox{for all $p\leq n$}                 \\
\end{align*}
where $\framep[n+2][p]$, $\framep[n+1][p]$ and $\restrf[n+1][p]$, as
well as $\painting[n+2][p]$, $\painting[n+1][p]$ and $\restrp[n+1][p]$
are now appropriate definitions.
...
So far, this means that we are able to specify a dependent structure of the form:
\begin{align*}
  \framep[n][p] & : \U & \mbox{for all $p\leq n$}                                                                                      \\
  \painting[n][p] & : \framep[n][p] \rightarrow \U & \mbox{for all $p\leq n$}                                                          \\
...
\end{align*}
which can appropriately be reformulated using sequences for appropriate definitions of $\frametype[n][0,...,p]$, $\paintingtype[n][0,...,p]$ and $\restrftype[n][0,...,p]$, since the dependencies of $\restrf[n][p]$ into $\restrf[n][0,...,p-1]$ makes it a very dependent function~\cite{Hickey96}:
\begin{align*}
...
In general, the types of sequences such as $\restrf[n][0,...,p]$ will be nested since components, such as $\restrf[n][p]$, may depend on previous components of the sequence. So, the type of a sequence of length $p$ will be expressed as a $p$-iterated $\Sigma$-type. In the case of $\framep$ and $\painting$, there will be however no dependency, so an ordinary product can be used. If $\framep[n][0,...,p]$ is such a sequence of length $p+1$, we write $\framep[n][0,...,p-1]$ for its immediate subsequence of length $p$ and $\framep[n][p]$ for its last component, and similarly for all such sequences.
...
Assuming $\framep[n][0,...,p-1]$ and $\painting[n][0,...,p-1]$,
we can mutually define the types $\restrftype[n][0,...,p-1]$ of frame
restrictions together with $\framep[n+1][0,...p]$ as a function
from $\restrf[n][0,...,p-1]$, using auxiliary definitions for the
types and definitions of layers:
\begin{equation*}
  \fontsize{7.2}{9}\selectfont
  \begin{array}{llcl}
    \framep[n+1][0,...,(p+1)-1]                                                                    &
...
\begin{align*}
  ...
  \left(\begin{array}{lll}
          ...
          \end{array}\right)
\end{align*}
so that the type of $\framep[n+1][0,...,p]$ can be shortened into $\depstype[n][0,..,p-1] \rightarrow \frametype[n+1][0,...,p]$ and similarly for $\layertype[n][p]$ and $\layer[n][p]$.
...
\begin{equation*}
  ...
  \begin{array}{llcl}
    \painting[n+1][0,...,p]   &
    (\fulldeps[n-1])        & :      & \paintingtype[n+1][0,...,p](\framep[n+1][0,...,p](\fulldeps.\deps[n-1][0,...,p])) \\
    \painting[n+1][0,...,0-1] &
    E                       & \defeq & \unittype                                                                     \\
    \painting[n+1][0,...,p]   &
    (\fulldeps[n-1])
                            & \defeq &
    (\painting[n+1][0,...,p-1](\fulldeps[n-1]),\painting[n+1][p](\fulldeps[n-1]))                                        \\
  \end{array}
\end{equation*}
...
Thanks to $\restrptype[n][0,...,p-1]$, we can mutually define $\restrf[n+1][0,...,p]$ and $\cohftype[n][0,...,p-1]$ from ``full dependencies'', that is from $\framep[n][0,...,n]$, $\painting[n][0,...,n]$, $\restrf[n][0,...,n]$ and some $E:\framep[n+1][n](\deps[n][0,...,n-1]) \rightarrow \U$, using abbreviations in passing for $\restrl$ and $\restrltype$:
...
\begin{equation*}
  ...
  \begin{array}{llcl}
    ...
    \multicolumn{3}{l}{
      ...
      \restrftype[n+1][0,...,p]
      \left(\begin{array}{l}
                \framep[n+1][0,...,p](\fulldeps.\deps[n][0,...,p])   \\
                \painting[n+1][0,...,p](\fulldeps[n]) \\
              \end{array}\right)
    }                                                                                                                                                        \\
    \restrf[n+1][0,...,(0+1)-1]                                                        &
    \left(\begin{array}{l}
    ...
    \left\{\begin{array}{l}
      ...
      \left(\begin{array}{l}
                \framep[n+1][p](\fulldeps.\deps[n][0,...,p-1])          \\
                \painting[n+1][0,...,p](\fulldeps[n]) \\
                \restrf[n+1][0,...,p]
                \left(\begin{array}{l}
                  \fulldeps[n]          \\
    ...
    \left(\begin{array}{l}
              \fulldeps[n]:\fulldepstype[n]                                        \\
              \restrp[n][0,...,p]:\restrptype[n][0,...,p](\fulldeps[n]) \\
...
\\
    ...
    & \defeq &
    \left\{\begin{array}{l}
    \Pi q \leq n. \Pi \epsilon. \Pi d: \framep[n+1][p].\\
    \left(
        \begin{array}{l}
    ...
    \left(\begin{array}{l}
              ...
              \cohf[n][0,...,p]     \\
            \end{array}\right)                                                     & \defeq &
    \lambda q\leq n. \lambda \epsilon.\,\lambda d.\lambda l.\overrightarrow{\cohf[n][p][q][q][\epsilon][\omega](d)}{\restrp[n][p][q][\epsilon](\restrf[n][p][q][\epsilon]\left(\begin{array}{l}
                  \fulldeps[n] \\
                  \restrp[n][0,...,p-1]       \\
                  \cohf[n][0,...,p-1]
                \end{array}\right)(d))(l \omega)}                          \\
  \end{array}
\end{equation*}
...
\begin{align*}
  ...
  \left(\begin{array}{lcl}
          ...
          \end{array}\right)
\end{align*}
so that the type of $\restrf[n+1][0,...,p]$ can be rephrased as $\Pi \depscohs[n][0,...,p-1]:\depscohstype[n][0,..,p-1]. \restrftype[n+1][0,...,p]
      \left(\begin{array}{l}
                \framep[n+1][0,...,p](\fulldeps.\deps[n][0,...,p])   \\
                \painting[n+1][0,...,p](\fulldeps[n]) \\
              \end{array}\right)$, and similarly for $\restrltype$ and $\restrl$. Later, we shall write $\depscohs.\deps$, $\depscohs.\restrp$, and $\depscohs.\cohf$ for the corresponding projections of an element $\depscohs$ in $\depscohstype$. By abuse of notation, we also write $E$ for an inhabitant of $\depscohstype[n][0,..,0-1]$ (standing for a non trivial inhabitant of $\fulldeps[n]$).

It is also convenient to build $\deps$ from $\depscohs$ at the
previous level:
\begin{align*}
  \overline{\deps}(\depscohs[n][0,...,p-1]:\depscohstype[n][0,...,p-1]) & :      & \depstype[n+1][0,...,p] \\
...
\begin{equation*}
  ...
  \begin{array}{llcl}
    \restrp[n+1][0,...,p]   &
    (\fulldepscohs[n])        & :      & \restrptype[n+1][0,...,p](\fulldepscohs.\depscohs.\fulldeps[n])) \\
    \restrp[n+1][0,...,0-1] &
    E  & \defeq & \unittype                                                                   \\
    \restrp[n+1][0,...,p]   &
    (\fulldepscohs[n])
...
\begin{equation*}
  ...
  \begin{array}{llcl}
    ...
    \cohptype[n][0,...,(p+1)-1] & (\fulldepscohs[n])                           & \defeq & \cohptype[n][0,...,p-1](\fulldepscohs[n]) \times \left(
    \begin{array}{l}
...
\begin{equation*}
  ...
  \begin{array}{llcl}
    \cohp[n+1][0,...,p]   &
    (\fulldepscoht[n])        & :      & \cohptype[n+1][0,...,p](\fulldepscoht.\depscoht.\fulldepscohs[n])) \\
    \cohp[n+1][0,...,0-1] &
    E  & \defeq & \unittype                                                                   \\
    \cohp[n+1][0,...,p]   &
    (\fulldepscoht[n])
...
Armed with all the constructions and assuming enough $E:\framep[n][n-1]
\rightarrow \U$ given, we can now define $\framep[n][p]$,
$\painting[n][p]$, $\restrf[n][p]$, $\restrp[n][p]$, $\cohf[n][p]$,
and $\cohp[n][p]$ for all $n$ and $p$. It is indeed enough to build
$\fulldepscoht[n]$ by induction on $n$:
\begin{itemize}
\item $\fulldepscohs[n+1]$ is $\overline{\fulldepscohs}(\fulldepscoht[n])$
\item $\cohp[n+1][0,...,n]$ is given from $\fulldepscoht$ as above
\item $\coht[n+1][0,...,n]$ is given by uniqueness of identity proofs
\end{itemize}

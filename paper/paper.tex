...
We give in this section a uniform definition of semi-simplicial and semi-cubical sets which we believe is folklore. The uniformity allows the definition of a notion of $\nu$-sets that subsumes semi-simplicial and semi-cubical sets.
...
\section{Indexed representation and parametricity\label{sec:indexed-param}}
In this section, we introduce some preliminaries on interpreting sets in type theory, followed by a small section on the indexed presentation, before relating it to parametricity in \ref{sec:rel-param}.
...
\subsection{Relating to parametricity\label{sec:rel-param}}
...
Our construction involves several details, some of them being visible only in the Coq formalization. A description at an informal level which we estimate to be however mathematically precise enough can be found in Sections~\ref{sec:ett} and~\ref{sec:itt}, while technical issues with the mechanization are explained in~\ref{sec:mech}.

The whole definition is highly dependent, with dependencies in
inequality proofs and in coherence conditions. In Section~\ref{sec:ett} and Tables~\ref{tab:coind}, \ref{tab:core}, \ref{tab:frames}, \ref{tab:faces}, \ref{tab:coh}, we
give a definition in Extensional Type Theory, which roughly
corresponds to a level of reasoning ``à la set theory'' where coherence
conditions are thought as ``holding on the nose'' without having to
mention them explicitly once they are justified. In Section~\ref{sec:itt} and Tables~\ref{fulltab:coind}, \ref{fulltab:core}, \ref{fulltab:frames}, \ref{fulltab:faces}, \ref{fulltab:coh}, we
give a definition in Intentional Type Theory, where transporting along
the coherence conditions is made explicit. This makes the definition
more involved, but at the same time helps to understand the
specificities of coherence conditions.
...
So far, we have not paid attention to the fact that we have a dependent type, shown as a $\Sigma$. Let us be more precise about this requirement. First, $\fullframe[n]$ depends on all $X_i$ up to $n - 1$. So, we need to package up $X_i$, for $i < n$, into a nesting of $\Sigma$-type, which we abbreviate as $\X[][<n][]$. This allows us to give the type $\X[][<n][] \rightarrow \U$ to $\fullframe[n]$. Then, for $D: \X[][<n][]$, representing an initial prefix of $X_0, X_1, \ldots X_{n - 1}$, the indexed set $X_n$ has type $\fullframe[n](D) \rightarrow \U$. Thus, $\framep[n][p]$, $\layer[n][p]$ and $\painting[n][p]$ also depend on $D$. We can then refine the previous equation by showing the dependencies on $D$. In particular, $X_n$ is just $D.\tl$.
\begin{align*}
   & \framep[n][p](D) \defeq                                                                                      \\

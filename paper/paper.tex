...
\begin{align*}
  ...
  \seqr{}{\Gamma \vdash A : \Type_m \qquad \Gamma, a:A \vdash B(a) : \Type_m \qquad \Gamma, a:A, b:B \vdash f(a,b):A \qquad \Gamma \vdash u:A}
  {\Gamma \vdash Stream_{A,B,f}\, u : \Type_m}                                                                                                                                          \\
  \mbox{\emph{introduction and eliminations}}                                                                                                                                           \\
  \seqr{}{\Gamma \vdash u : A \qquad \Gamma \vdash v : B(u) \qquad \Gamma \vdash s : Stream_{A,B,f}\,f(u,v)}{\Gamma \vdash \{this:=v; next:=s\} : Stream_{A,B,f}\, u}                   \\
  \seqr{}{\Gamma \vdash t : Stream_{A,B,f}\, u}{\Gamma \vdash t.this : B(u)}                                                                                                            \\
  \seqr{}{\Gamma \vdash t : Stream_{A,B,f}\, u}{\Gamma \vdash t.next : Stream_{A,B,f}\, f(u,v) \equiv u}                                                                                \\
  \mbox{\emph{computation}}                                                                                                                                                             \\
  \seqr{}{\Gamma \vdash u : A \qquad \Gamma \vdash v : B(u) \qquad \Gamma \vdash s : Stream_{A,B,f}\,f(u,v)}{\Gamma \vdash \{this:=v; next:=s\}.this \equiv v : B(u)}                   \\
  \seqr{}{\Gamma \vdash u : A \qquad \Gamma \vdash v : B(u) \qquad \Gamma \vdash s : Stream_{A,B,f}\,f(u,v)}{\Gamma \vdash \{this:=v; next:=s\}.next \equiv s : Stream_{A,B,f}\,f(u,v)} \\
\end{align*}
...
Table~\ref{tab:coind} describes the type of a $\nu$-set at level $m$ in indexed form, as a dependent stream of type families representing the limit of $n$-truncated $\nu$-sets (there, the recursive equation $\Xfrom{m}{n}{D} \defeq \Sigma R:\Xcomp{m}{n}[D=\D]. \Xfrom{m}{n+1}[D=\pair{D}{R}]$ defines the stream $Stream_{\Sigma n.\Xto{m}{n},\lambda (n, \D)\Xcomp{m}{n}[D=\D],\lambda ((n, D),R).(n+1,\pair{D}{R})} (n, \D)$ mentioned in Section~\ref{?}): $\Xfrom{m}{n}$ denotes an infinite sequence $X_{n+1}, X_{n+2}, \ldots$ dependent on a $n$-truncated $\nu$-set, $\Xto{m}{n}$, so that, when $n$ is $0$, it denotes a full $\nu$-set, written $\Xp{m}$. This is made possible because the $0$-truncated $\nu$-set, $\Xto{m}{0}$, is degenerated: it is an empty family, and there is thus only one $0$-truncated $\nu$-set, namely the canonical inhabitant $\kstar$ of $\unittype$.

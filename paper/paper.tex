...
Our contribution takes a different direction from traditional contributions to the field of HoTT: although it is a proposed model of HoTT, it uses \emph{unicity of identity proofs} (UIP) instead of univalence to finish off the final coherence condition; the two axoims are mutually incompatible.
...
\begin{listing}[H]
  \begin{minted}[linenos]{coq}
    ...
    Defined.
  \end{minted}
  \caption{↕ definition, when $\leq$ is defined using an inductive approach}
\end{listing}
...
\begin{listing}[H]
  ...
  \end{minted}
  \caption{↕ definition, when $\leq$ is defined using a nested implication}
\end{listing}
...
\mint{coq}/Notation "↕" := (Hnm : n <= m) (Hmp : m <= p) : n <= p/
\mint{coq}/Notation "↑" := (Hnm : n <= m) : n <= S m/
\mint{coq}/Notation "↓" := (Hnm : S n <= m) : n <= m/
\mint{coq}/Notation "⇓" := (Hnm : S n <= S m) : n <= m/
\mint{coq}/Notation "⇑" := (Hnm : n <= m) : S n <= S m/
\subsection{Yoneda.v}

...
An informal description of what should semi-simplicial sets in indexed form be was given in~\cite{UFwiki2012}. Several implementations in type theory were later proposed~\cite{voevodsky12,herbelin15,part15,altenkirch16}. In \cite{hr25}, a construction of both augmented semi-simplicial and semi-cubical sets in \emph{indexed form} was given, as a special case of iterated parametricity in indexed form. Instead of the following illustration of semi-cubical sets in fibered form:
...
We recall from~\cite{hr25} how to formally define a type characterizing the sequences $E_0, E_1, \ldots$. The type theory we consider is a basic type theory with $\Pi$-types, $\Sigma$-types, Martin-L\"of identity type $=$, universes $U_m$, a $\unittype$ type, natural numbers, a definitionally proof-irrelevant inequality on natural numbers, as well as streams. The non-dependent variants of $\Pi$ and $\Sigma$ are written $\rightarrow$ and $\times$ respectively. The canonical inhabitant of $\unittype$ is written $\kstar$ and the subtype of types $T$ in $U_m$ satisfying $UIP : \Pi xy : T. \Pi pq : x = y. p = q$ is written $\U[m]$. The definitional proof-irrelevance of inequality can be replaced with propositional proof-irrelevance with a few extra transports.

Let us sketch how to define iterated parametricity in indexed form. The dependency of $E_{n+1}$ into all $E_p$ for $p \leq n$ makes the family $E$ a \emph{very dependent function} \cite{Hickey96}. The standard way to express it is then as a stream whose $(n+1)$-th element $E_{n+1}$ is dependent on the bundle of all previous elements of the stream. We write $\Xto{m}{n}$ for the bundle of all $E_0, \ldots, E_{n-1}$, and we write $\Xfrom{m}{n}{D}$ for the residual stream $E_{n}, E_{n+1}, \ldots$, which is precisely dependent on the bundle $E:\Xto{m}{n}$ of previous $E_0, \ldots, E_{n-1}$. The type of $E_n$, itself also dependent on a bundle $D:\Xto{m}{n}$, is denoted $\Xcomp{m}{n}[D=\D]$, as shown on tables~\ref{tab:coind} and~\ref{tab:core}. When $n$ is $0$, $\Xto{m}{0}$ is degenerated, that is reduced to the canonical inhabitant $\kstar$. Then, $\Xfrom{m}{0}{(\kstar)}$ represents the type of the full stream $E_0, E_1, \ldots$.
...
In the formalisation accompanying~\cite{hr25}, a rather heavy justification of the well-foundedness of the construction was given which we remind in section~\ref{sec:previous-formalisation}. In this section, we give a more elegant evidence.

Indeed, to build $\framep[n+1]$ from $\framep[n]$, it happens to be enough to assume the following built at level $n$, for all $p \leq n$:
...
\begin{equation*}
  ...
  \end{array}
\end{equation*}
and to build it at level $n+1$. This is however not so easy because some components of this structure, e.g. $\restrf$ and further fields, depend on $\framep[n+1]$, that is, it already depends on the ability of building the $\framep$ component of the structure at level $n+1$. We are thus facing an interdependency between specifying the structure assumed at level $n$ and defining from it the structure at level $n+1$. So, we have to proceed in order: specifying $\framep[n]$, from which $\painting[n]$ can be specified, from which $\framep[n+1]$ can be built and thus $\restrf[n+1]$ be specified. Now, the definition of $\framep[n+1][p]$, by induction on $p$, itself depends on $\restrf[n][p-1]$ whose type depends in turn on $\framep[n+1][p-1]$. This means that $\framep[n+1][p]$ and the type of $\restrf[n][p]$ actually have to be mutually defined.
...
Using this recursive definition of $\framep[n+1][\lbrack 0,p \rbrack]$ and $\restrf[n][\lbrack 0,p-1 \rbrack]$, we are able to specify the three first component of the expected dependent structure, for $p \leq n$.
...
When $n$ is $0$, we write $E$ to denote an element of $\fulldepstype[0]$ where all other components are degenerate.
...
$E$. $\deps[n][\lbrack 0,n \rbrack]$ come from the assumptions at level $n$, and to build the following:
\begin{itemize}
...
\section{Comparison with previous formalisation\label{sec:previous-formalisation}}

To address the dependency of $\restrf[n][p]$ into $\framep[n+1][p]$ and the extra dependency of $\cohf[n][p]$ into $\framep[n+2][p]$ and $\restrf[n+1][p]$ (and similarly for $\painting$), the formalization given in \cite{hr25} was assuming all of $\framep[n][p]$, $\framep[n+1][p]$, $\framep[n+2][p]$, $\restrf[n][p]$, $\restrf[n+1][p]$ and $\cohf[n][p]$ together with propositional equations defining $\framep[n+1][p]$, $\framep[n+2][p]$ and $\restrf[n+1][p]$ (and similarly for $\painting$). The induction step then amounted to build $\framep[n+3][p]$, $\restrf[n+2][p]$ and $\cohf[n+1][p]$ (and similarly for $\painting$). This paper can be seen as the result of analysing how dependencies are finely organised so that the propositional equations in \cite{hr25} are replaceable by definitional equations of $\framep[n+1][p]$, $\framep[n+2][p]$ and $\restrf[n+1][p]$ (and similarly for $\painting$).

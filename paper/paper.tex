...
% Adjust font size
\newcommand{\smfontsize}{\fontsize{7.6}{9}\selectfont}
...
\keywords{Parametricity, Semi-simplicial sets, Semi-cubical sets, Formalization, Rocq}
...
\paragraph*{Parametricity and its .}
...
Notably, the definition of $\restrl$ relies on an equality expressing the commutation of the composition of two $\restrf$. The proof of this commutation is made explicit in table~\ref{tab:coh} using proof-term notations\footnote{We however omit type conversions, as if working in extensional type theory.}. The proof is by recursion on the structure of $\framep$, $\layer$, and $\painting$. This is what $\cohf$ does using auxiliary proofs $\cohl$ and $\cohp$. Note that for a fixed constant $n$, relying on the evaluation rules of type theory, the coherence conditions degenerate to a reflexivity proof, so that the construction builds an effective sequence of types not mentioning coherences anymore.
...
In particular, the dependency of $\framep[n+1][\lbrack 0,p-1 \rbrack]$ within the type of $\restrf[n][\lbrack 0,p-1 \rbrack]$ combined with the dependency of $\restrf[n][\lbrack 0,p-1 \rbrack]$ within the definition of $\framep[n+1][\lbrack 0,p \rbrack]$ requires a mutual definition the type of $\restrf[n][\lbrack 0,p-1 \rbrack]$ together with $\framep[n+1][p]$ (or $\framep[n+1][\lbrack 0,p \rbrack]$) as a function from $\restrf[n][\lbrack 0,p-1 \rbrack]$.

So far, using this recursive definition of  $\framep[n+1][\lbrack 0,p \rbrack]$ and $\restrf[n][\lbrack 0,p-1 \rbrack]$, we are able to specify the three first component of the expected dependent structure, for $p \leq n$:
\begin{equation*}
  \smfontsize
  \begin{array}{lcl}
    \framep[n][p]   & : & \U                                                \\
...
To build this structure at level $n+1$, from level $n$, it is
enough to assume
\begin{equation*}
  E:\framep[n+1][n+1](\deps[n][\lbrack 0,n \rbrack]) \rightarrow \U
\end{equation*}
where $\deps[n][\lbrack 0,n \rbrack]$ come from the assumptions at level $n$, and to build the following:
\begin{itemize}
...
The construction we have presented differs from the Rocq formalisation in \href{https://github.com/artagnon/bonak}{the bonak repository} in the following ways:
\begin{itemize}
  \item The main difference between the informal presentation and the Rocq formalisation is that the formalisation is not parameterised by $n$ but instead by $k \defeq n - p$. This makes the presentation a bit less intuitive since the dimension has to be mentally recomputed by adding $p$ to $k$, but technically, it happens to provide a smoother formalisation. That is, the interesting information when building some component up to $p$ is not, computationally speaking, the dimension but the distance from to the expected dimension.
  ...
  \item In section \ref{sec:detailed-construction}, we left implicit the management of inequality proofs occurring in restrictions and coherence conditions. In practice, we stated inequalities as strict propositions~\cite{gilbert19}, or propositions whose proofs are all definitionally equal. This simplified the formalisation a bit.

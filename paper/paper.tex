...
\usepackage{multirow, minted, tabularx, booktabs, setspace, makecell, caption, xstring, pdflscape}
...
% More magic: we extend xkeyval with a \appendmask macro to pass to \bonaksetkeys
\makeatletter
\def\bonak@mask{}

\def\bonak@appendmask[#1]{
  \XKV@addtolist@n\bonak@mask{\g@addto@macro{\XKV@prefix\XKV@tfam}{@}#1}
}
\def\appendmask{\XKV@testopta{\XKV@testoptc\bonak@appendmask}}
\makeatother

% Now we create \bonaksetkeys
\makeatletter
\def\bonaksetkeys#1[#2]#3[#4]#5{
  \def\bonak@pfx{\g@addto@macro{\XKV@prefix\XKV@tfam}{@}}
  \XKV@for@n{\bonak@mask}\bonak@tempa{
    \IfBeginWith{\bonak@tempa}{\bonak@pfx}{
      \XKV@addtolist@n\XKV@na{\StrBehind{\bonak@tempa}{\bonak@pfx}}}{}
  }
  % Finally, call XKV@setkeys
  \let\bonak@tempa{\XKV@testopta{\XKV@testoptc\XKV@setkeys}}
  \expandafter\bonak@tempa\expandafter\XKV@na
}
\makeatother

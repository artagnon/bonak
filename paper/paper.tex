...
\newcommandx{\depsp}[8][2,3,4,5,6,7,8]{
  \ensuremath{\mathsf{\color{russian-green}{deps}}_{
    \joinargs[\mathsf{\color{indian-yellow}{#1}}][#2][#3][#4]}^{\joinargs[#5][#6][#7]}}
}

\newcommandx{\depstypep}[8][2,3,4,5,6,7,8]{
  \ensuremath{\mathsf{\color{russian-green}{DEPS}}_{
    \joinargs[\mathsf{\color{indian-yellow}{#1}}][#2][#3][#4]}^{\joinargs[#5][#6][#7]}}
}
...
\newcommandx{\deps}[2][1,2]{\depsp{}[][][][#1][#2][][]}
\newcommandx{\fulldeps}[1][1]{\depsp{full}[][][][#1][][][]}
\newcommandx{\depscohs}[2][1,2]{\depsp{cohs}[][][][#1][#2][][]}
\newcommandx{\fulldepscohs}[1][1]{\depsp{fullcohs}[][][][#1][][][]}
\newcommandx{\depscoht}[2][1,2]{\depsp{coht}[][][][#1][#2][][]}
\newcommandx{\fulldepscoht}[1][1]{\depsp{fullcoht}[][][][#1][][][]}
\newcommandx{\depstype}[2][1,2]{\depstypep{}[][][][#1][#2][][]}
\newcommandx{\fulldepstype}[2][1,2]{\depstypep{full}[][][][#1][][][]}
\newcommandx{\depscohstype}[2][1,2]{\depstypep{cohs}[][][][#1][#2][][]}
\newcommandx{\fulldepscohstype}[1][1]{\depstypep{fullcohs}[][][][#1][][][]}
\newcommandx{\depscohttype}[2][1,2]{\depstypep{coht}[][][][#1][#2][][]}
\newcommandx{\fulldepscohttype}[1][1]{\depstypep{fullcoht}[][][][#1][][][]}
...
Since $\restrf[n][p]$ is referring to $\framep[n][p]$ and $\framep[n-1][p]$, and $\cohf[n][p]$ is referring to $\restrf[n][p]$ and $\restrf[n-1][p]$, thus also to $\framep[n][p]$, $\framep[n-1][p]$ and $\framep[n-2][p]$, we built all of these 6 components at the same time. But equations were also needed to state the properties of $\framep[n][p]$, $\framep[n-1][p]$ and $\restrf[n][p]$ so that we were building by induction on $n+p$ (that is on $n$, using the convention of \cite{hr25}), for any $D:\Xto{m}{n}$, the following block of information:

\begin{align*}
  \framep[n][p]      & : \U                                                                                                                 & \mbox{for all $p\leq n$}   \\
  \framep[n-1][p]    & : \U                                                                                                                 & \mbox{for all $p\leq n-1$} \\
  \framep[n-2][p]    & : \U                                                                                                                 & \mbox{for all $p\leq n-2$} \\
  \restrf[n][p]      & : \Pi q.\, \Pi \omega.\, \framep[n][p] \rightarrow \framep[n-1][p]                                                   & \mbox{for all $p\leq n$}   \\
  \restrf[n-1][p]    & : \Pi q.\, \Pi \omega.\, \framep[n-1][p] \rightarrow \framep[n-2][p]                                                 & \mbox{for all $p\leq n-1$} \\
  \cohf[n][p]        & : \Pi qr\epsilon\omega.\, \restrf[n-1][p][\epsilon][q] \circ \restrf[n][p][\omega][r] =                                                           \\
                     & \;\;\;\;\restrf[n-1][p][\omega][r] \circ \restrf[n][p][\epsilon][q+1]                                                & \mbox{for all $p\leq n$}   \\
  \mathsf{hypF0}     & : \framep[n][0]         = \unitpoint
  \\
  \mathsf{hypFS}     & : \framep[n][p+1]       = \Sigma d: \framep[n+1][p]. \Pi \omega.\, \painting[n][p] (\restrf[n+1][p](d))              & \mbox{for all $p\leq n-1$} \\
  \mathsf{hypPrevF0} & : \framep[n-1][0]                                                                                       = \unitpoint                              \\
  \mathsf{hypPrevFS} & : \framep[n-1][p+1] = \Sigma d: \framep[n][p]. \Pi \omega.\, \painting[n-1][p] (\restrf[n][p](d))                    & \mbox{for all $p\leq n-2$} \\
  \mathsf{hypRF0}    & : \restrf[n][0]        = \ldots \texttt{depending on hypF0}                                                                                       \\
  \mathsf{hypRFS}    & : \restrf[n][p+1]      = \ldots \texttt{depending on hypFS and hypPrevFS}                                            & \mbox{for all $p\leq n-1$} \\
  \ldots             & \;\;\;\;\texttt{fields for painting, restrf, cohp}
\end{align*}

In the current paper and associated formalisation, we switch the point
of view, take $\framep[n-2][p]$ $\painting[n-2][p]$, $\restrf[n-1][p]$
and $\restrp[n-1][p]$ as references and rename them respectively as
$\framep[n][p]$, $\painting[n][p]$, $\restrf[n][p]$ and
$\restrp[n][p]$. We replace the equational properties of $\framep[n][p]$,
$\framep[n-1][p]$, $\restrf[n][p]$, $\painting[n][p]$,
$\painting[n-1][p]$ and $\restrp[n][p]$ with definitions, renaming them
respectively to $\framep[n+2][p]$, $\framep[n+1][p]$,
$\restrf[n+1][p]$, $\painting[n+2][p]$, $\painting[n+1][p]$ and
$\restrp[n+1][p]$. This means that we build instead by induction a block of the form:
...
\begin{align*}
  ...
  \cohf[n][p]     & : \Pi qr\epsilon\omega.\, \restrf[n][p][\epsilon][q] \circ \restrf[n+1][p][\omega][r] =                                                \\
                  & \;\;\;\;\restrf[n][p][\omega][r] \circ \restrf[n+1][p][\epsilon][q+1]                                       & \mbox{for all $p\leq n$} \\
...
Moreover, the definition of $\framep[n+1][p]$ depends on $\framep[n+1][p-1]$, so all of $\framep[n][k]$, $\painting[n][k]$ and $\restrf[n][k]$ for $k< p$ need to be known, which we respectively abbreviate as $\framep[n][0,...,p-1]$, $\painting[n][0,...,p-1]$ and $\restrf[n][p-1]$.
...
\begin{center}
  \fontsize{7.2}{9}\selectfont
  \begin{tikzcd}
    \framep[n+1][0,...,p] \arrow[d] \arrow[ddr]\\
    \painting[n][0,...,p-1] \arrow[d, dotted] \\
    \framep[n][0,...,p-1] &
    \restrf[n][0,...,p-1] \arrow[l, dotted] \arrow[uul, dotted, "\mbox{$[0,...,p-1]$}"', near start, shift right=2mm, hook']\\
  \end{tikzcd}
\end{center}

In particular, the mutual dependency on one side of $\framep[n][0,...,p-1]$ within the type of $\restrf[n][0,...,p-1]$ and on the other side of $\restrf[n][0,...,p-1]$ within the definition of $\framep[n][0,...,p]$ requires to mutually define the type of $\restrf[n][0,...,p-1]$ together with $\framep[n][p]$ (or $\framep[n][0,...,p]$) as a \emph{function} from $\restrf[n][0,...,p-1]$.

So far, using this recursive definition of  $\framep[n][0,...,p-1]$ and $\restrf[n][0,...,p-1]$, we are able to specify the three first component of the exected dependent structure:
\begin{align*}
  \framep[n][p]             & : \U                                & \mbox{for all $p\leq n$} \\
  \painting[n][p]           & : \framep[n][p] \rightarrow \U      & \mbox{for all $p\leq n$} \\
  \restrf[n][p]             & : \Pi q \leq n+p.\, \framep[n+1][p]
  \left(\begin{array}{l}
            \framep[n][0,...,p-1]   \\
            \painting[n][0,...,p-1] \\
            \restrf[n][0,...,p-1]   \\
...
\end{align*}
which can appropriately be reformulated using sequences for appropriate definitions of $\frametype[n][0,...,p]$, $\paintingtype[n][0,...,p]$ and $\restrftype[n][0,...,p]$, since the dependencies of $\restrf[n][p]$ into $\restrf[n][0,...,p-1]$ makes it a very dependent function~\citep{Hickey96}:
\begin{align*}
  \framep[n][0,...,p]   & : \frametype[n][0,...,p]                         \\
  \painting[n][0,...,p] & : \paintingtype[n][0,...,p](\framep[n][0,...,p]) \\
  \restrf[n][0,...,p]   & : \restrftype[n][0,...,p]
  \left(\begin{array}{l}
            \framep[n][0,...,p-1]   \\
            \painting[n][0,...,p-1] \\
            \restrf[n][0,...,p-1]   \\
...
The type of sequences such as $\restrf[n][0,...,p]$ will be represented by iterated $\Sigma$-types since each $\restrf[n][p]$ may depend on the previous components of the sequence. In the case of $\framep$ and $\painting$, there will be however no dependency, so an ordinary product can be used to represent $\framep[n][0,...,p]$ and $\painting[n][0,...,p]$. Later on, we will write $\framep[n][0,...,p-1]$ for the immediate subsequence of length $p$ of $\framep[n][0,...,p]$ and $\framep[n][p]$ for its last component, and similarly for $\painting[n][0,...,p]$, $\restrf[n][0,...,p]$, etc.

\section{The construction}

\subsection{Specifying frames}

Assuming $\framep[n][0,...,p]$ for $p \leq n$ only requires to define the type of frames, which are just finite products of $\U$:
...
The types $\paintingtype[n][0,...,p]$ of $\painting[n][0,...,p]$ for $p \leq n$
are type families over frames, so $\framep[n][0,...,p]$ needs to be assumed
beforehand:
...
Assuming $\framep[n][0,...,p-1]$ and $\painting[n][0,...,p-1]$ for $p \leq n+1$, we can
mutually define the types $\restrftype[n][0,...,p-1]$ of frame
restrictions together with $\framep[n+1][0,...p]$ as a function from
$\restrf[n][0,...,p-1]$, introducing also abbreviations for the types
and definitions of layers, as well as for the combination of sequences
of $\framep[n][0,..,p-1]$, $\painting[n][0,...,p-1]$ and
$\restrf[n][0,...,p-1]$ in one block, which we call
$\depstype[n][0,...p-1]$:
...
\begin{equation*}
  ...
  \begin{array}{llcl}
    ...
    \framep[n+1][0,...,p]     &
    \multicolumn{3}{l}{
    (\deps[n][0,...,p-1]:\depstype[n][0,...,p-1]) :\frametype[n+1][0,...,p]}         \\
    \framep[n+1][0,...,0]     &
    \unitpoint                    & \defeq & (\unitpoint,unit)                     \\
    \framep[n+1][0,...,p+1] &
    \deps[n][0,...,p]               & \defeq &
    \left(\begin{array}{l}
              \framep[n+1][0,...,p]
...
Defining $\painting[n+1][p]$ for $p \leq n+1$, in addition to the type dependency in $\framep[n+1][p]$, requires the definition of $\painting[n][p-1]$, thus also $\framep[n][p-1]$ by typing dependency, as well as $\restrf[n][p-1]$. Since $\painting[n+1][p]$ also depends on $\painting[n+1][p+1]$ when $p<n+1$, these dependencies are actually between $\painting[n+1][p,...,n+1]$, $\painting[n][p,...,n]$, $\framep[n+1][p,...,n+1]$, $\framep[n][p,...,n]$ and $\restrf[n][p,...,n]$. Also, for the case $p=n$, this requires the assumption of an inhabitant $X_{n}$ of $\nu$Set$^{=n}$. Finally, $\framep[n+1][p,...,n+1]$, $\framep[n+1][p,...,n]$ and $\restrf[n][p,...,n]$ respectively depend on the earlier values $\framep[n+1][0,...,p-1]$, $\framep[n][0,...,p-1]$ and $\restrf[n][0,...,p-1]$.  Below, we use dashed underline and dashed arrows for dependencies in subconstructions, here in the construction of $\framep$ at level $n$ and $\restrftype$ at level $n$:
\begin{center}
  \fontsize{7.2}{9}\selectfont
  \begin{tikzcd}
    & & \painting[n+1][p,...,n+1] \arrow[dd, bend right=70] \arrow[dddr] \arrow[d, dotted]\\
    \udensdash{\framep[n+1][0,...,p-1]} \arrow[d, dashed] \arrow[ddr, dashed] & &
    \udensdash{\framep[n+1][p,...,n+1]} \arrow[ll, dotted] \arrow[d, dashed] \arrow[ddr, dashed, shift right=2mm] \\
    \painting[n][0,...,p-1] \arrow[d, dotted] & &
    \painting[n][p,...,n-1] \arrow[ll, dotted] \arrow[d, dotted] \\
    \framep[n][0,...,p-1] &
    \udensdash{\restrf[n][0,...,p-1]} \arrow[l, dotted] \arrow[uul, dotted, shift right=2mm, hook'] &
    \framep[n][p,...,n] \arrow[ll, dotted, bend left=15] &
    \udensdash{\restrf[n+1][p,...,n]},X_n \arrow[ll, dotted, bend left=15] \arrow[l, dotted] \arrow[uul, dotted, hook']\\
  \end{tikzcd}
\end{center}

Let us introduce another abbreviation to combine a full $\deps[n][0,...,n]:\depstype[n][0,...,n]$ with a family $E:\framep[n+1][n+1](\deps[n][0,...,n])\rightarrow \U$:
\begin{align*}
  \fulldepstype[n] \defeq
  \left(\begin{array}{lll}
            \deps[n][0,...,n] & : & \depstype[n][0,...,n]                             \\
            E                   & : & \framep[n+1][n+1](\deps[n][0,...,n]) \rightarrow \U \\
          \end{array}\right)
\end{align*}
...
We can now formally define $\painting[n+1][p]$ by recursion on $n+1-p$:
\begin{equation*}
  \fontsize{7.2}{9}\selectfont
  \begin{array}{llcl}
    \painting[n+1][p]      &
    ...
    \painting[n+1][n+1]      &
    \fulldeps[n]           & \defeq & \fulldeps.E             \\
    \painting[n+1][p](p\leq n) &
    \fulldeps[n]           & \defeq & d \mapsto
    \left(\begin{array}{l}
              \Sigma l:\layer[n][p](\fulldeps.\deps[n][0,...,p]). \\
              \painting[n+1][p+1]
              (\fulldeps[n])(d,l)                                 \\
            \end{array}\right) \\
  \end{array}
...
From which we can deduce $\painting[n+1][0,...,p]$ for $p \leq n+1$ by a second recursion, this time on $p$:

\begin{equation*}
  \fontsize{7.2}{9}\selectfont
  \begin{array}{llcl}
    \painting[n+1][0,...,p]         &
    (\fulldeps[n]:\fulldepstype[n]) & :      & \paintingtype[n+1][0,...,p](\framep[n+1][0,...,p](\fulldeps.\deps[n-1][0,...,p])) \\
    \painting[n+1][0,...,0-1]       &
    \fulldeps[n]                    & \defeq & \unittype                                                                         \\
    \painting[n+1][0,...,p]         &
    \fulldeps[n]
                                    & \defeq &
    (\painting[n+1][0,...,p-1](\fulldeps[n-1]),\painting[n+1][p](\fulldeps[n]))                                                  \\
  \end{array}
\end{equation*}
...
\begin{equation*}
  ...
  \begin{array}{llcl}
    \restrptype[n][0,...,p-1]       &
    (\fulldeps[n]:\fulldepstype[n]) & :      & \U                                                    \\
    \restrptype[n][0,...,0-1]       & E      & \defeq                                    & \unittype \\
    \restrptype[n][0,...,(p+1)-1]   &
    (\fulldeps[n])
                                    & \defeq & \restrptype[n+1][0,...,p-1] \times \left(
    \begin{array}{l}
        \Pi q\leq n.\Pi \epsilon.\Pi d:\framep[n+1][p](\fulldeps.\deps[n][0,...,p-1]). \\
        \painting[n+1][p]
        (\fulldeps[n])(d) \rightarrow                                                  \\ \fulldeps.\deps.\painting[n][p](\fulldeps.\deps.\restrf[n][p][q][\epsilon](d))\end{array}
    \right)                                                                                          \\
  \end{array}
\end{equation*}
...
Thanks to $\restrptype[n][0,...,p-1]$ for $p \leq n+1$, we can mutually define $\restrf[n+1][0,...,p]$ and $\cohftype[n][0,...,p-1]$ from $\framep[n][0,...,n]$, $\painting[n][0,...,n]$, $\restrf[n][0,...,n]$, $\restrptype[n][0,...,p-1]$ and some $E:\framep[n+1][n+1](\deps[n][0,...,n]) \rightarrow \U$, abbreviating these dependencies and the dependency in a $\cohf[n][0,...,p-1]$ as $\depscohstype[n][0,...,p-1]$, and using also abbreviations in passing for $\restrl$ and $\restrltype$. It is also convenient to build $\deps[n+1][0,...,p]$ from $\depscohs[n][0,...,p-1]$, so that we can now build $\framep[n+2][p]$ one step further. Thus, we also include this abbreviation in the definition:
\begin{equation*}
  \fontsize{7.2}{9}\selectfont
  \begin{array}{llcl}
    ...
    \overline{\deps}                &
    \multicolumn{3}{l}{
    (\depscohs[n][0,...,p-1]:\depscohstype[n][0,...,p-1]) : \depstype[n+1][0,...,p]}                                                                                                                             \\
    \overline{\deps}                & \depscohs[n][0,...,p-1] & \defeq                                                         &
    \left(\begin{array}{l}
              \framep[n+1][0,...,p](\depscohs.\fulldeps.\deps[n][0,...,p-1]) \\
              \painting[n+1][0,...,p](\depscohs.\fulldeps[n])                \\
              \restrf[n+1][0,...,p](\depscohs[n][0,...,p-1])                 \\
            \end{array}\right)                                                                                                                                         \\
    \restrf[n+1][0,...,p]     &
    \multicolumn{3}{l}{
      ...
      \restrftype[n+1][0,...,p]
      \left(\begin{array}{l}
                \framep[n+1][0,...,p](\depscohs.\fulldeps.\deps[n][0,...,p]) \\
                \painting[n+1][0,...,p](\depscohs.\fulldeps[n])              \\
              \end{array}\right)
    }                                                                                                                                                                                                            \\
    \restrf[n+1][0,...,0]     &
    ...
    \restrf[n+1][0,...,p+1] &
    (\depscohs[n][0,..,p])          & \defeq                  &
    \left(\begin{array}{l}
              \restrf[n+1][0,...,p][q][\epsilon]
              (\depscohs[n][0,..,p-1]),                 \\
              \lambda q.\lambda \epsilon.\lambda (d,l). \\
              \left(\restrf[n][p][q][\epsilon]
                (\depscohs[n][0,..,p-1])(d), \restrl[n][p][q][\epsilon]
              (\depscohs[n][0,..,p])(d)(l)\right)       \\
            \end{array} \right)                                                                                                                                    \\
    \cohftype[n][0,...,p-1]         &
    \multicolumn{3}{l}{
      \left(\begin{array}{l}
    ...
    \left\{\begin{array}{l}
             ...
             \left(\begin{array}{l}
                ...
                \restrp[n][0,...,p-1] \\
              \end{array}\right).                          \\
             \Pi q\leq n.\,\Pi r\leq q.\,\Pi \epsilon.\,\Pi \omega.\,\Pi d:\framep[n+2][p]
             (\overline{\deps}\left(\begin{array}{l}
                                 \fulldeps[n]          \\
                                 \restrp[n][0,...,p-1] \\
                                 \cohf[n][0,...,p]
                               \end{array}\right)          . \\
    ...
    \depscohs[n][0,..,p-1]
                                    & \defeq                  &
    \left\{\begin{array}{l}
             \Pi q \leq n. \Pi \epsilon. \Pi d: \framep[n+2][p](\overline{\deps}(\depscohs[n][0,..,p-1])).\, \\
             \layer[n+1][p](\overline{\deps}(\depscohs[n][0,..,p-1])(d) \rightarrow                          \\
             \layer[n][p](\fulldeps.\deps[n][0,...,p])(\restrf[n][p]
             (\depscohs[n][0,..,p-1])(d))
           \end{array}\right.                                                                                                       \\
    \restrl[n][p]                   &
    \multicolumn{3}{l}{
      (\depscohs[n][0,..,p]:\depscohstype[n][0,..,p]) :
    \restrltype[n][p] (\depscohs[n][0,..,p])}                                                                                                                                                                    \\
    \restrl[n][p]                   &
    \depscohs[n][0,..,p]            & \defeq                  &
    \lambda q\leq n. \lambda \epsilon.\,\lambda d.\lambda l.\overrightarrow{\cohf[n][p][q][q][\epsilon][\omega](d)}{\restrp[n][p][q][\epsilon](\restrf[n][p][q][\epsilon](\depscohs[n][0,..,p-1])(d))(l \omega)} \\
  \end{array}
\end{equation*}

By abuse of notation, we also write $E$ for an inhabitant of $\depscohstype[n][0,..,0-1]$ (standing for a non trivial inhabitant of $\fulldeps[n]$).

We need also the notion of full dependencies at the level of coherence conditions, that combines a full $\depscohs[n][0,...,n]$ with a set $E:\framep[n+2][n+2](\overline{\deps}(\depscohs[n][0,...,n])) \rightarrow \U$:
\begin{align*}
  \fulldepscohstype[n] \defeq
  \left(\begin{array}{lcl}
            \depscohs[n][0,...,n-1] & : & \depscohstype[n][0,...,n]                                                \\
            E                       & : & \framep[n+2][n+2](\overline{\deps}(\depscohs[n][0,...,n]))\rightarrow \U \\
          \end{array}\right)
\end{align*}
TODO: say the convention on projection in the Introduction
...
\begin{align*}
  \overline{\fulldeps}(\fulldepscohs[n]:\fulldepscohstype[n]) & :      & \fulldepstype[n+1] \\
  \overline{\fulldeps}(\fulldepscohs[n])                      & \defeq &
  \left(\begin{array}{l}
            \overline{\deps}(\fulldepscohs.\depscohs[n][0,...,n]) \\
            \fulldepscohs.E                     \\
          \end{array}\right)                             \\
\end{align*}
...
\begin{equation*}
  ...
  \begin{array}{llcl}
    ...
    \left(
    \begin{array}{l}
        \Pi q\leq n. \Pi \epsilon. \Pi d:\framep[n+2][p]
        (\overline{\deps}(\fulldepscohs.\depscohs[n][0,...,p-1])). \\
        \painting[n+2][p]
        (\overline{\fulldeps}(\fulldepscohs[n]))(d) \rightarrow    \\
        \painting[n+1][p](\fulldepscohs.\depscohs.\fulldeps[n])
        (\restrf[n+1][p](\fulldepscohs.\depscohs[n][0,...,p-1])(d))\end{array}\right)                                                        \\
    \restrp[n+1][n+1]                         &
    (\fulldepscohs[n])                      & \defeq & \lambda q\leq n.\lambda \epsilon. \lambda d. \lambda (l,c). l\epsilon \\
    \restrp[n+1][p](p\leq n)                    &
    (\fulldepscohs[n])                      & \defeq & \lambda q\leq n.\lambda \epsilon. \lambda d. \lambda (l,c).
    \left(\begin{array}{l}
              \restrl[n][p][q][\epsilon](\fulldepscohs.\depscohs[n][0,...,p])(d)(l), \\
              \restrp[n+1][p+1][q][\epsilon](\fulldepscohs[n])(d,l)(c)               \\
            \end{array}\right)                                             \\
  \end{array}
\end{equation*}

From which we can deduce $\restrp[n+1][0,...,p]$ for $p \leq n+1$ again by a second recursion, this time on $p$:

\begin{equation*}
  \fontsize{7.2}{9}\selectfont
  \begin{array}{llcl}
    \restrp[n+1][0,...,p]                   &
    (\fulldepscohs[n]:\fulldepscohstype[n]) & :      & \restrptype[n+1][0,...,p](\fulldepscohs.\depscohs.\fulldeps[n])) \\
    \restrp[n+1][0,...,0-1]                 &
    \fulldepscohs[n]                        & \defeq & \unittype                                                        \\
    \restrp[n+1][0,...,p]                   &
    \fulldepscohs[n]
                                            & \defeq &
    (\restrp[n+1][0,...,p-1](\fulldepscohs[n]),\restrp[n+1][p](\fulldepscohs[n]))                                       \\
  \end{array}
\end{equation*}

\subsection{Specifying painting coherence laws}

At this stage, we are then able to define $\restrf[n+1][0,...,p]$, $\restrp[n+1][0,...,p]$ and $\cohftype[n][0,...,p-1]$ for $p \leq n+1$
from $\depscohs[n][0,...,p-1]$ at the previous level. To go further, we need to define the
sequence $\cohptype[n][0,...,p-1]$ of types of painting coherence laws:

\begin{equation*}
  \fontsize{7.2}{9}\selectfont
  \begin{array}{llcl}
    \cohptype[n][0,...,p-1]     & (\fulldepscohs[n]:\fulldepscohstype[n]) & :      & \U                                               \\
    \cohptype[n][0,...,0-1]     & \unitpoint                              & \defeq & \unittype                                        \\
    \cohptype[n][0,...,(p+1)-1] & (\fulldepscohs[n])                      & \defeq & \cohptype[n][0,...,p-1](\fulldepscohs[n]) \times \\
                                &                                         &        & \;\;\;\;\left\{
    \begin{array}{l}
      \Pi q\leq n.\,\Pi r\leq q.\,\Pi \epsilon.\,\Pi \omega.\,\Pi d:\framep[n+2][p](\overline{\deps}(\fulldepscohs.\depscohs[n][0,...,p-1])).\,\Pi c:\painting[n+2][p](\overline{\fulldeps}(\fulldepscohs[n])) \\
      \fulldepscohs.\depscohs.\restrp[n][p][q][\epsilon](\restrf[n+1][p](d))(\restrp[n+1][p][r][\omega]
      (\fulldepscohs[n])(d)(c)) =                                                                                                                                                                              \\
      \fulldepscohs.\depscohs.\restrp[n][p][r][\omega](\restrf[n+1][p](d))(\restrp[n+1][p][q+1][\epsilon]
      (\fulldepscohs[n])(d)(c))
    \end{array}
    \right.                                                                                                                           \\
  \end{array}
...
Thanks to $\cohptype[n][0,...,p-1]$, we can mutually define $\cohf[n+1][0,...,p]$ and $\cohttype[n][0,...,p-1]$ for $p \leq n+1$ from $\fulldepscohs[n][0,...,p-1]$, abbreviating this and the dependency in a $\coht[n][0,...,p-1]$ as $\depscohttype[n][0,...,p-1]$, and using also abbreviations in passing for $\cohl$ and $\cohltype$. It is also convenient to build $\depscohs[n+1][0,...,p]$ from $\depscoht[n][0,...,p-1]$, so that we can now build $\framep[n+3][p]$. Thus, we also include this abbreviation in the definition:
  ...
  \begin{array}{llcl}
    ...
    \left(\begin{array}{lll}
              ...
              \left(\begin{array}{l}
                ...
                \end{array}\right)                                           \\
            \end{array}\right)                                                                                                                                                \\
    \overline{\depscohs}                                             & (\depscoht[n][0,...,p-1]:\depscohttype[n][0,...,p-1]) & :                                                                                 & \depscohstype[n+1][0,...,p] \\
    \overline{\depscohs}                                             & \depscoht[n][0,...,p-1]                             & \defeq                                                                            &
    \left(\begin{array}{l}
              \overline{\deps}(\depscoht.\fulldepscohs.\depscohs[n][0,...,p-1]) \\
              \restrp[n+1][p](\depscoht.\fulldepscohs[n])                       \\
              \cohf[n+1][p](\depscoht[n][0,...,p-1])                            \\
            \end{array}\right)                                                                                                                                                                \\
    \cohf[n+1][0,...,p]                                              &
    (\depscoht[n][0,...,p-1]:\depscohttype[n][0,...,p-1])                          & :                                                 & \cohftype[n+1][0,...,p]
           \left(\begin{array}{l}
              \overline{\fulldeps}(\depscoht.\fulldepscohs[n])                         \\
              \restrp[n+1][0,...,p](\depscoht.\fulldepscohs[n]) \\
            \end{array}\right) \\
                                                                                         \\
    \cohf[n][0,...,0]                                          &
    \unitpoint                                                       & \defeq                                            & (\unitpoint,\lambda q.\lambda r. \lambda\epsilon. \lambda\omega.\lambda \_.\refl)                               \\
    \cohf[n][0,...,p+1]                                      &
    (\depscoht[n][0,...,p-1])                                               & \defeq                                            &
    \left(\begin{array}{l}
              \cohf[n+1][0,...,p]
              (\depscoht[n]),                                                \\
    ...
    \left(\begin{array}{l}
            ...
            \end{array}\right)                                                                                                                           \\
    \cohl[n][p]                                                      &
    (\depscoht[n][0,...,p-1]:\depscohttype[n][0,...,p-1])                          & :                                                 &
    \begin{array}{l}
...
We need also the notion of full dependencies at the level of the coherence conditions of dimension 2, that combines a full $\depscoht[n][0,...,n]$ with a set $E:\framep[n+3][n+3](\overline{\deps}(\overline{\depscohs}(\depscoht[n][0,...,n]))) \rightarrow \U$:
\begin{align*}
  \fulldepscohttype[n] \defeq
  \left(\begin{array}{lll}
            \depscoht[n][0,...,n] & : & \depscohttype[n][0,...,n]                                                    \\
            E                       & : & \framep[n+3][n+3](\overline{\deps}(\overline{\depscohs}(\depscoht[n][0,...,n])))\rightarrow \U \\
          \end{array}\right)
\end{align*}
...
\begin{align*}
  \overline{\fulldepscohs}(\fulldepscoht[n]:\fulldepscohstype[n]) & :      & \fulldepscohstype[n+1] \\
  \overline{\fulldepscohs}(\fulldepscoht[n])                   & \defeq &
  \left(\begin{array}{l}
            \overline{\depscohs}(\fulldepscoht.\depscoht[n][0,...,n]) \\
            \fulldepscoht.E                     \\
          \end{array}\right)                      \\
\end{align*}
...
The last key construction we need is $\cohp[n+1][p]$, which is by recursion on $n-p$:

\begin{equation*}
  \fontsize{7.2}{9}\selectfont
  \begin{array}{llcl}
    \cohp[n+1][p]                           &
    (\fulldepscoht[n]:\fulldepscohttype[n]) & :      &
    \left(
    \begin{array}{l}
    ...
    \left(\begin{array}{l}
              \cohl[n][p][q][r][\epsilon][\omega](\fulldepscoht.\depscoht[n][0,...,p-1])(d)(l), \\
              \cohp[n+1][p+1][q][r][\epsilon][\omega](\fulldepscoht[n])(d,l))(c)                \\
            \end{array}\right)                                                                \\
  \end{array}
\end{equation*}

From which we can deduce $\cohp[n+1][0,...,p]$ again by a second recursion, this time on $p$:

\begin{equation*}
  \fontsize{7.2}{9}\selectfont
  \begin{array}{llcl}
    \cohp[n+1][0,...,p]                     &
    (\fulldepscoht[n]:\fulldepscohttype[n]) & :      & \cohptype[n+1][0,...,p](\fulldepscoht.\depscoht.\fulldepscohs[n])) \\
    \cohp[n+1][0,...,0-1]                   &
    \fulldepscohs[n]                        & \defeq & \unittype                                                          \\
    \cohp[n+1][0,...,p]                     &
    \fulldepscoht[n]
                                            & \defeq &
    (\cohp[n+1][0,...,p-1](\fulldepscoht[n]),\cohp[n+1][p](\fulldepscoht[n]))                                             \\
  \end{array}
\end{equation*}
...
\begin{align*}
  \rotatebox[origin=c]{90}{\depscohs[n][0,...,n-1]}\begin{cases}
                                                     \rotatebox[origin=c]{90}{\deps[n][0,...,n-1]}\begin{cases}
                                                   \framep[n][0,...,n-1]   : \frametype[n][0,...,n-1]                           \\
                                                   \painting[n][0,...,n-1] : \paintingtype[n][0,...,n-1](\framep[n][0,...,n-1]) \\
                                                   \restrf[n][0,...,n-1]   : \restrftype[n][0,...,n-1]
                                                   \left(\begin{array}{l}
                \framep[n][0,...,n-1]   \\
                \painting[n][0,...,n-1] \\
              \end{array}\right)
                                                 \end{cases} \\
                                                     E:\framep[n+1][n]
                                                     (\deps[n][0,...,n-1])\rightarrow \U                                                                                       \\
                                                     \restrp[n][0,...,n-1](E) : \restrptype[n][0,...,n-1]
                                                     \left(\begin{array}{l}
              \deps[n][0,...,n-1] \\
              E                   \\
            \end{array}\right)                                                                                                 \\
                                                     \cohf[n][0,...,n-1](E)   : \cohftype[n][0,...,n-1]
                                                     \left(\begin{array}{l}
              \deps[n][0,...,n-1]                 \\
              E:\framep[n+1][n]
              (\deps[n][0,...,n-1])\rightarrow \U \\
            ...
            \end{array}\right)
                                                   \end{cases} \\
  E': \framep[n+2][n+1](\overline{\deps}
  (\depscohs[n][0,...,n-1])\rightarrow \U                                                                                                                                                  \\
  \cohp[n][0,...,n-1]
  \left(\begin{array}{l}
            E  \\
            E' \\
...
enough to assume $E:\framep[n+1][n](\deps[n]) \rightarrow \U$ (where
$\deps[n]$ come from the assumptions at level $n$) and to build the
following:
\begin{itemize}
  \item $\framep[n+1][0,...,n]$ comes from $\deps[n]$ which holds by assumptions (using $\framep[n][0,...,n-1]$, $\painting[n][0,...,n-1]$ and $\restrf[n][0,...,n-1]$)
  \item $\painting[n+1][0,...,n]$ comes from $\fulldeps[n]$ which holds by assumptions (also using $E$)
  \item $\restrf[n+1][0,...,n]$ comes from $\depscohs[n][0,...,n-1]$ which holds by assumptions (additionally using the assumptions of $\restrp[n][0,...,n-1]$ and $\cohf[n][0,...,n-1]$)
  \item $\restrp[n+1][0,...,n]$ assuming $E':\framep[n+2][n+1](\overline{\deps}(\depscohs[n][0,...,n-1])) \rightarrow \U$ comes from $\fulldepscohs[n]$ which holds by assumptions (additionally using $E'$)
  \item $\cohf[n+1][0,...,n]$ assuming $E'$ comes from $\depscoht[n][0,...,n-1]$ which holds by assumptions (additionally using the assumption of $\cohp[n][0,...,n-1]$ and proving $\coht[n][0,...,n-1]$ by uniqueness of identity proofs)
  \item $\cohp[n+1][0,...,n]$ assuming $E'$ and $E'': \framep[n+3][n+2](\overline{\deps}(\overline{\depscohs}(\depscoht[n][0,...,n-1]))) \rightarrow \U$ comes from $\fulldepscoht[n][0,...,n-1]$ which holds by assumptions (additionally using $E''$)
\end{itemize}

Therefore, assuming a construction of $\framep[n][n-1]$ from
$D:\Xto{m}{n}$, we are able to build a $\framep[n+1][n]$ from $D$
extended with some $E:\framep[n+1][n](\deps[n]) \rightarrow \U$. This
is enough to build $\Xcomp{m}{n}(D)$, for $D:\Xto{m}{n}$, and thus
$\Xp{m}$, by starting with an empty $D$.

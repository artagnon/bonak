...
The well-foundedness of the construction from tables~\ref{tab:coind} to~\ref{tab:coh} was justified in the previous work by induction on a large structure involving equational reasoning. However, the equational reasoning can be avoided, and that it is enough to assume the following structure built at level $n$, for all $p \leq n$, and to build it at level $n+1$.
...
The price to pay for being free of equations is, however, mixing the specification of the induction hypothesis and the construction of induction step. To begin, $\restrf$ depends on $\framep[n+1]$: it already depends on the ability of building the $\framep$ component of the structure at level $n+1$. There is therefore an interdependency between specifying the structure assumed at level $n$ and defining from it the structure at level $n+1$. We have to proceed in order: specifying $\framep[n]$, from which $\painting[n]$ can be specified, from which $\framep[n+1]$ can be built, and thus allowing $\restrf[n+1]$ be specified. Now, the definition of $\framep[n+1][p]$, by induction on $p$, itself depends on $\restrf[n][p-1]$, whose type depends in turn on $\framep[n+1][p-1]$. Hence, $\framep[n+1][p]$ and the type of $\restrf[n][p]$ have to be mutually defined.

The definition of $\framep[n+1][p]$ also depends, via $\layer[n][p-1]$, on $\painting[n][p-1]$. Both via $\painting[n][p-1]$ and $\restrf[n][p-1]$, it depends also on $\framep[n][p-1]$ by typing dependency. This requires us to assume $\framep[n][p-1]$ and $\painting[n][p-1]$. Moreover, the definition of $\framep[n+1][p]$ depends on $\framep[n+1][p-1]$, so $\framep[n][k]$, $\painting[n][k]$, and $\restrf[n][k]$, for $k < p$, need to be known. We respectively abbreviate these as $\framep[n][\lbrack 0,p-1 \rbrack]$, $\painting[n][\lbrack 0,p-1 \rbrack]$ and $\restrf[n][\lbrack 0,p-1 \rbrack]$. Schematically, using dotted arrows for dependencies in the type and plain arrows for dependencies in the definition, this gives the following graph of dependencies for $p \leq n+1$.
...
\begin{equation*}
  ...
  \end{array}
\end{equation*}
which can be reformulated using sequences for appropriate definitions of $\frametype[n][\lbrack 0,p \rbrack] \defeq \overbrace{\mbox{$p+1 \times$}}{\U \times \ldots \times \U}$, $\paintingtype[n][\lbrack 0,p \rbrack] \defeq (\framep[n][0] \rightarrow \U) \times \ldots \times (\framep[n][p] \rightarrow \U)$, and $\restrftype[n][\lbrack 0,p \rbrack]$:
\begin{equation*}
  \smfontsize
  \begin{array}{lcl}
...
To address the dependency of $\restrf[n][p]$ in $\framep[n+1][p]$, and that of $\cohf[n][p]$ in $\framep[n+2][p]$ and $\restrf[n+1][p]$ without having to mix the specification of the induction hypothesis and the construction of the next step, the previous work~\cite{hr25} was assuming all of $\framep[n][p]$, $\framep[n+1][p]$, $\framep[n+2][p]$, $\restrf[n][p]$, $\restrf[n+1][p]$, and $\cohf[n][p]$, together with propositional equations defining $\framep[n+1][p]$, $\framep[n+2][p]$, and $\restrf[n+1][p]$. The induction step then built $\framep[n+3][p]$, $\restrf[n+2][p]$ and $\cohf[n+1][p]$. This work can be seen as the result of analysing how dependencies are finely organised so that those propositional equations are replaced by definitional ones $\framep[n+1][p]$, $\framep[n+2][p]$ and $\restrf[n+1][p]$. That is, the induction hypothesis in \cite{hr25} combines assumptions of the form $\framep[n+1][p] : \U$ with equations of the form $\framep[n+1][0] = \unittype$ and $\framep[n+1][p+1] = \Sigma d:\framep[n+1][p].\forall \epsilon.\painting[n][p](\restrf[n][p][0][\epsilon])$. On the other side, these are replaced by a definition of $\frame[n+1][p]$.

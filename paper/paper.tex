...
An informal description of semi-simplicial sets in indexed form be was given in~\cite{UFwiki2012}. Several formal definitions in type theory were later proposed~\cite{voevodsky12,herbelin15,part15,altenkirch16}. In the previous work~\cite{hr25}, a construction of both augmented semi-simplicial and semi-cubical sets in \emph{indexed form} was given, as a special case of iterated parametricity in indexed form. Instead of semi-cubical sets in fibred form, a family of relevant relations was used.
...
The well-foundedness of the construction from tables~\ref{tab:coind} to~\ref{tab:coh}, was justified in the previous work by induction on a large structure involving equational reasoning. However, the equational reasoning can be avoided, and that it is enough to build by induction on $n$ the following shorter structure:
...
The price to pay for this conciseness is mixing the specification of the induction hypothesis and the construction of the induction step. On the outset, $\restrf$ depends on $\framep[n+1]$: it already depends on the ability of building the $\framep$ component of the structure at level $n+1$. There is an interdependency between specifying the structure assumed at level $n$ and defining from it the structure at level $n+1$. We have to proceed in a specific order: specifying $\framep[n]$, from which $\painting[n]$ can be specified, from which $\framep[n+1]$ can be built, from which $\restrf[n+1]$ can be specified. Now, the definition of $\framep[n+1][p]$, which proceeds by induction on $p$, itself depends on $\restrf[n][p-1]$ whose type depends in turn on $\framep[n+1][p-1]$. Hence, $\framep[n+1][p]$ and the type of $\restrf[n][p]$ have to be mutually defined.
...
Using this recursive definition of $\framep[n+1][\lbrack 0,p \rbrack]$ and $\restrf[n][\lbrack 0,p-1 \rbrack]$, we are thus able to add to the structure the specification of its third component, for $p \leq n$.
...
\begin{equation*}
  ...
  \end{array}
\end{equation*}
which can be reformulated using sequences for appropriate definitions of $\frametype[n][\lbrack 0,p \rbrack]$, $\paintingtype[n][\lbrack 0,p \rbrack]$, and $\restrftype[n][\lbrack 0,p \rbrack]$:
\begin{equation*}
  \smfontsize
  \begin{array}{lcl}
...
Assuming $\framep[n][\lbrack 0,p-1 \rbrack]$ for $p \leq n+1$ only requires defining the type of frames, which are just finite products of $\U$.

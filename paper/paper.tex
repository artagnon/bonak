...
Remark: If we weren't working in \U, but in \HGpd, we would need to prove one more higher-dimensional coherence, and if we were working in \Type, we would need to prove arbitarily many higher-dimensional coherences. For the unfamiliar reader, $\HGpd$ is the subset of types $A$ such that for all $x$ and $y$ in $A$, $x =_{HGpd} y$ is in \U. See \cite{herbelin15} for a discussion on the need for recursive higher-dimensional coherence conditions in formulating higher-dimensional structures in type theory.
...
Our formalization is done in the intensional type theory flavor of the calculus of inductive constructions. Working in intensional type theory necessitates explicitly supplying a number of type equalities, which hold definitionally in extensional type theory.

The construction is obviously on induction on $n$, but $\restrf[n]$ depends on $\framep[n]$ and $\framep[n-1]$, while $\cohf[n]$ depends on $\framep[n]$, $\framep[n-1]$, and $\framep[n-2]$. A priori, this necessitates well-founded induction. Due to technical difficulties experienced with Coq, we chose a different route; in practice, since we only need to keep track of three levels, we built separate data structures for the levels, with dependencies.

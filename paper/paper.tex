...
\begin{table}[H]
  \begin{tabularx}{\linewidth}{cYcYYc}
    ...
    \makecell{unary}  & \makecell{indexed}      & \makecell{non-iterated} & \makecell{proof-irrelevant} & \makecell{predicate}          \\
                      ...
                      &                         & \makecell{iterated}     & \makecell{relevant}         & \emph{this work}              \\
    \midrule
...
An informal description of what should semi-simplicial sets in indexed form be was given in~\cite{UFwiki2012}. Several formal definitions in type theory were later proposed~\cite{voevodsky12,herbelin15,part15,altenkirch16}. In \cite{hr25}, a construction of both augmented semi-simplicial and semi-cubical sets in \emph{indexed form} was given, as a special case of iterated parametricity in indexed form. Instead of the following illustration of semi-cubical sets in fibered form,
...
In the previous work~\cite{hr25}, a rather heavy justification of the well-foundedness of the construction was given which we recall in section~\ref{sec:previous-work}. Now, we observe that, to build $\framep[n+1]$ from $\framep[n]$, it is enough to assume the following structure built at level $n$, for all $p \leq n$, and to build it at level $n+1$.
...
The fields following $\restrf$ depend on $\framep[n+1]$, which is to say that it already depends on the ability of building the $\framep$ component of the structure at level $n+1$. So, there is an interdependency between specifying the structure assumed at level $n$ and defining from it the structure at level $n+1$. We have to proceed in order: specifying $\framep[n]$, from which $\painting[n]$ can be specified, from which $\framep[n+1]$ can be built, and thus allowing $\restrf[n+1]$ be specified. Now, the definition of $\framep[n+1][p]$, by induction on $p$, itself depends on $\restrf[n][p-1]$ whose type depends in turn on $\framep[n+1][p-1]$. So, $\framep[n+1][p]$ and the type of $\restrf[n][p]$ have to be mutually defined.
...
\begin{equation*}
  ...
  \end{array}
\end{equation*}
which can be reformulated using sequences for appropriate definitions of $\frametype[n][\lbrack 0,p \rbrack] \defeq \overbrace{\mbox{$p+1$ times}}{\U \times \ldots \times \U}$, $\paintingtype[n][\lbrack 0,p \rbrack] \defeq (\framep[n][0] \rightarrow \U) \times \ldots \times (\framep[n][p] \rightarrow \U)$, and $\restrftype[n][\lbrack 0,p \rbrack]$:
\begin{equation*}
  \smfontsize
  \begin{array}{lcl}
...
Assuming $\framep[n][\lbrack 0,p-1 \rbrack]$ and $\painting[n][\lbrack 0,p-1 \rbrack]$ for $p \leq n+1$, we can mutually define the types $\restrftype[n][\lbrack 0,p-1 \rbrack]$ of frame restrictions together with $\framep[n+1][\lbrack 0,p \rbrack]$ as a function from $\restrf[n][\lbrack 0,p-1 \rbrack]$. We introduce an abbreviation, $\depstype[n][\lbrack 0,p-1 \rbrack]$, for the combination of sequences of $\framep[n][\lbrack 0,p-1 \rbrack]$, $\painting[n][\lbrack 0,p-1 \rbrack]$ and $\restrf[n][\lbrack 0,p-1 \rbrack]$. By abuse of notation, once a $\deps:\depstype$ is assumed, we freely refer to $\framep$, $\painting$, $\restrf$. We write $\unitpoint$ for the only inhabitant of $\depstype[n][0]$.
...
To address the dependency of $\restrf[n][p]$ in $\framep[n+1][p]$, and that of $\cohf[n][p]$ in $\framep[n+2][p]$ and $\restrf[n+1][p]$, the previous work~\cite{hr25} was assuming all of $\framep[n][p]$, $\framep[n+1][p]$, $\framep[n+2][p]$, $\restrf[n][p]$, $\restrf[n+1][p]$, and $\cohf[n][p]$, together with propositional equations defining $\framep[n+1][p]$, $\framep[n+2][p]$, and $\restrf[n+1][p]$. The induction step then built $\framep[n+3][p]$, $\restrf[n+2][p]$ and $\cohf[n+1][p]$. This work can be seen as the result of analysing how dependencies are finitely organised so that those propositional equations are replaced by definitional ones $\framep[n+1][p]$, $\framep[n+2][p]$ and $\restrf[n+1][p]$.

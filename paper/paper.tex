...
\begin{example}[$\DeltaPlus^1$]
  In dimension $1$, the standard augmented semi-simplex can be pictured as a point, where the color is kept implicit. This point is the identity in $\Hom(1, 1)$; it is hence shown as a singleton $\kstar$:
...
\begin{definition}[\Cube]
  $\Cube$ is defined similarly to $\DeltaPlus$, except that we take $0$, $1$ and $\star$, instead of $0$ and $\kstar$. We use $0$ and $1$ to indicate opposite faces of a cube.
...
\begin{example}[$\Cube^0$]
  $\Cube^0$ is $\Hom(0, 0)$, or the empty set:
...
\begin{example}[$\Cube^1$]
  $\Cube^1$ is two points and a line:
  \begin{equation*}
    \begin{tikzcd}

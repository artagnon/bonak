...
\begin{align*}
  \framep[n][p]   & : \U[m]                                                                                                     & \mbox{for all $p\leq n$} \\
  \painting[n][p] & : \framep[n][p] \rightarrow \U[m]                                                                           & \mbox{for all $p\leq n$} \\
...
Assuming $\framep[n][0,...,p]$ only requires to define the type of frames, which are just finite products of $\U$:
...
The types $\paintingtype[n][0,...,p]$ of $\painting[n][0,...,p]$
...
Assuming $\framep[n][0,...,p-1]$ and $\painting[n][0,...,p-1]$, we can
...
Defining $\painting[n+1][p]$, in addition to the type dependency in $\framep[n+1][p]$, requires the definition of $\painting[n][p-1]$, thus also $\framep[n][p-1]$ by typing dependency, as well as $\restrf[n][p-1]$. Since $\painting[n+1][p]$ also depends on $\painting[n+1][p+1]$ when $p<n+1$, these dependencies are actually between $\painting[n+1][p,...,n+1]$, $\painting[n][p,...,n]$, $\framep[n+1][p,...,n+1]$, $\framep[n][p,...,n]$ and $\restrf[n][p,...,n]$. Also, for the case $p=n$, this requires the assumption of an inhabitant $X_{n}$ of $\nu$Set$^{=n}$. Finally, $\framep[n+1][p,...,n+1]$, $\framep[n+1][p,...,n]$ and $\restrf[n][p,...,n]$ respectively depend on the earlier values $\framep[n+1][0,...,p-1]$, $\framep[n][0,...,p-1]$ and $\restrf[n][0,...,p-1]$.  Below, we use dashed underline and dashed arrows for dependencies in subconstructions, here in the construction of $\framep$ at level $n$ and $\restrftype$ at level $n$:
\begin{center}
  \fontsize{7.2}{9}\selectfont
  \begin{tikzcd}
...
Let us introduce another abbreviation to combine a full $\deps[n][0,...,n-1]:\depstype[n][0,...,n-1]$ with a family $E:\framep[n+1][n](\deps[n][0,...,n-1])\rightarrow \U$:
\begin{align*}
  \fulldepstype[n] \defeq
  \left(\begin{array}{lll}
            \deps[n][0,...,n-1] & : & \depstype[n][0,...,n-1]                             \\
            E                   & : & \framep[n+1][n](\deps[n][0,...,n-1]) \rightarrow \U \\
          \end{array}\right)
\end{align*}
...
From which we can deduce $\painting[n+1][0,...,p]$ by a second recursion, this time on $p$:
...
At this stage, we are able to define $\framep[n+1][0,...,n]$, $\painting[n+1][0,...,n]$ and $\restrftype[n][0,...,n-1]$ from $\framep[n][0,...,n-1]$, $\painting[n][0,...,n-1]$ and $\restrf[n][0,...,n-1]$, that is from $\deps[n][0,...,n-1]$. To go further, we need to define the sequence $\restrptype[n][0,...,p]$ of types of painting restrictions:
...
Thanks to $\restrptype[n][0,...,p-1]$, we can mutually define $\restrf[n+1][0,...,p]$ and $\cohftype[n][0,...,p-1]$ from $\framep[n][0,...,n]$, $\painting[n][0,...,n]$, $\restrf[n][0,...,n]$, $\restrptype[n][0,...,p-1]$ and some $E:\framep[n+1][n](\deps[n][0,...,n-1]) \rightarrow \U$, abbreviating these dependencies and the dependency in a $\cohf[n][0,...,p-1]$ as $\depscohttype[n][0,...,p-1]$, and using also abbreviations in passing for $\restrl$ and $\restrltype$. It is also convenient to build $\deps[n+1][0,...,p]$ from $\depscohs[n][0,...,p-1]$, so that we can now build $\framep[n+2][p]$ one step further. Thus, we also include this abbreviation in the definition:
\begin{equation*}
  \fontsize{7.2}{9}\selectfont
  \begin{array}{llcl}
...
We need also the notion of full dependencies at the level of coherence conditions, that combines a full $\depscohs[n][0,...,n]$ with a set $E:\framep[n+2][n+1](\overline{\deps}(\depscohs[n][0,...,n-1])) \rightarrow \U$:
\begin{align*}
  \fulldepscohstype[n] \defeq
  \left(\begin{array}{lcl}
            \depscohs[n][0,...,n-1] & : & \depscohstype[n][0,...,n-1]                                                \\
            E                       & : & \framep[n+2][n+1](\overline{\deps}(\depscohs[n][0,...,n-1]))\rightarrow \U \\
          \end{array}\right)
\end{align*}
TODO: say the convention on projection in the Introduction
...
\begin{align*}
  ...
  \left(\begin{array}{l}
            \overline{\deps}(\fulldepscohs.\depscohs[n][0,...,n-1]) \\
...
At this stage, we are then able to define $\restrf[n+1][0,...,p]$, $\restrp[n+1][0,...,p]$ and $\cohftype[n][0,...,p]$
  ...
  \begin{array}{llcl}
    ...
    \left(\begin{array}{lll}
              ...
              \left(\begin{array}{l}
                ...
                \end{array}\right)                                           \\
            \end{array}\right)                                                                                                                                                \\
    \overline{\depscohs}                                             & (\depscoht[n][0,...,p]:\depscohttype[n][0,...,p]) & :                                                                                 & \depscohstype[n+1][0,...,p] \\
    \overline{\depscohs}                                             & \depscoht[n][0,...,p]                             & \defeq                                                                            &
    \left(\begin{array}{l}
...
We need also the notion of full dependencies at the level of the coherence conditions of dimension 2, that combines a full $\depscoht[n][0,...,n]$ with a set $E:\framep[n+3][n+2](\overline{\depscohs}(\depscoht[n][0,...,n-1])) \rightarrow \U$:
\begin{align*}
  \fulldepscohttype[n] \defeq
  \left(\begin{array}{lll}
            \depscoht[n][0,...,n-1] & : & \depscohttype[n][0,...,n-1]                                                    \\
            E                       & : & \framep[n+3][n+2](\overline{\depscohs}(\depscoht[n][0,...,n-1]))\rightarrow \U \\
          \end{array}\right)
\end{align*}
...
\begin{align*}
  ...
  \left(\begin{array}{l}
            ...
            \overline{\depscohs}(\fulldepscoht.\depscoht[n][0,...,n-1]) \\

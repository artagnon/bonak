...
A family of sets can commonly be represented in two ways: as a family properly speaking, indexed over a set or as a set together with a map specifying for each element of the set on which index it depends. In the latter case, the set associated to a given index is the fiber, i.e. inverse image, of the index under the map. The two presentations are equivalent, what can be formally stated in a type-theoretic language as a fibered/indexed correspondence~\footnote{At a discussion on the Zulip channel for the HoTT community, alternative names fibration/family correspondence and unbundled/bundled correspondence were proposed, among others. In our experience, the opposition fibered/indexed is spontaneously understood by many listeners. In particular, it echoes to the Grothendieck construction of fibered categories from indexed categories. The most elementary instance of the correspondence, with $\Type$ instead of $\U$, that is $(\Sigma S: \Type. (S \rightarrow T)) \simeq (T \rightarrow \Type)$, is sometimes called ``Grothendieck construction for dummies'' (its proof requires univalence, see~\cite{hottbook}).}:
\begin{equation*}
  \mbox{(fibered)}\qquad(\Sigma S: \U. (S \rightarrow T)) \simeq (T \rightarrow \U)\qquad \mbox{(indexed)}
\end{equation*}
where $\U$ represents the subset of types with a unique proof of equality, that is the type-theoretic counterpart of the category of sets (within some universe).

While a presheaf over an index category, i.e. a family of sets connected with maps according to the morphisms of the index category, can be seen as living on the indexed side of the correspondence, opposing to discrete Grothendieck fibrations on the fibered side, there are situations where a presheaf $F$ can also be seen as living on the fibered side of the correspondence. Indeed, when the index category is direct, i.e. with a downwards-well-founded collection of non-identity morphisms, as is the case of semi-simplicial and semi-cubical sets, each $F(X)$ serving as source of a map $F(X) \stackrel{F(f)}{\rightarrow} F(Y)$ can be turned into a family $F(X)$ indexed on $F(Y)$, and, since the maps are well-founded downward, this can be iterated from below, eventually producing a collection of families which we call an \emph{indexed presentation} of the presheaf over the direct category.

For instance, in the case of a semi-cubical set~\citep{grandis03,buchholtz17}, presented with $2n$ face maps from the set of $n$-cubes to the set of $(n-1)$-cubes, the standard presheaf approach prescribes a family of sets fibered the ones over the others (up to coherence conditions such). Formulated in type theory, it corresponds to:
...
On the other side, the indexed presentation of the same presheaf is a stratified sequence of families indexed on elements of families of lower rank, taking into account the coherence conditions to prevent duplications. Formulated in type theory, it corresponds to:
...
\subsection*{An iterated parametricity-based indexed construction}

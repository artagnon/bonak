...
\usepackage{multirow, minted, tabularx, environ, booktabs, setspace, makecell, caption, xkeyval, xstring, xpatch, stringstrings, ifthen, pdflscape}
...
\xpatchcmd{\XKV@setkeys}{\XKV@checksanitizea{#2}\XKV@resb}{
  ...
  \XKV@for@o\XKV@fams\XKV@tfam{
    ...
    \XKV@for@o\bonak@mask\XKV@tempa{
      ...
      \ifthenelse{\equal{\theresult}{0}}{}
      {
        ...
        \let\XKV@tempb\XKV@resb
        \let\KXV@resb\@empty
        \XKV@for@o\XKV@tempb\XKV@tempb{
          \XKV@getkeyname\XKV@tempb\XKV@tempc
          \ifthenelse{\equal{\XKV@tempc}{\thestring}}{}{
            % Unmasked
            \XKV@addtolist@o\XKV@resb\XKV@tempb
          }
        }
      }
    }
...
\NewEnviron{eqntable}[1]{
  \begin{table}[H]
    \small
    \begin{tabularx}{\textwidth}{@{}>{$}l<{$} >{$}c<{$} >{$}c<{$} >{\centering\arraybackslash}X @{}}
      \toprule
      \BODY
      \bottomrule
    \end{tabularx}
...
\NewDocumentCommand{\eqnline}{m m m m}{#1 & #2 & #3 & \ensuremath{#4} \\}

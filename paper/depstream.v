...
Check fun a d => eq_refl : (make a d).(next a) = make (f a (make a d).(this a)) (s a d).
...
Definition DepStreamProp (a:A) : Prop :=
  exists D : A->Prop, (D a /\ exists (v : forall a, D a -> B a), (forall a d, D (f a (v a d)))).

Definition this_prop a (x : DepStreamProp a) : B a :=
  let '(ex_intro _ D (conj d (ex_intro _ v s))) := x in v a d.

Definition next_prop a (x : DepStreamProp a) : DepStreamProp (f a (this_prop a x)) :=
  let '(ex_intro _ D (conj d (ex_intro _ v s))) := x in
  ex_intro _ D (conj (s a d) (ex_intro _ v s)).

Context (D:A->Prop) (v : forall a, D a -> B a) (s : forall a d, D (f a (v a d))).
...
Check fun a d => eq_refl : next_prop a (make_prop a d) = make_prop (f a (this_prop a (make_prop a d))) (s a d).
